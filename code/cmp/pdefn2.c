
/* This file was automatically generated by pclu. */

#include "pclu_err.h"
#include "pclu_sys.h"



/**** BEGIN PROCEDURE p_where ****/

extern errcode p_envOPget_peek1();
extern errcode p_envOPnext_token();
extern errcode p_clist();
extern errcode p_restrict();
extern errcode sequenceOPnew();
extern const struct REQS * const sequence_of_t_reqs;
extern struct OPS *expr_ops;
extern const OWN_req sequence_ownreqs;
extern struct OPS *sequence_ops;
struct OPS *sequence_of_expr_ops __CLU_COMMON;
extern struct OPS *string_ops;
extern struct OPS *sequence_of_expr_ops;
extern struct OPS *typespec_ops;
struct OPS *record_name_parms_type__ops __CLU_COMMON;
extern struct OPS *record_name_parms_type__ops;
struct OPS *sequence_of_record_name_parms_type__ops __CLU_COMMON;
extern struct OPS *int_ops;
extern struct OPS *sequence_of_record_name_parms_type__ops;
struct OPS *record_line_opers_ops __CLU_COMMON;
extern struct OPS *record_line_opers_ops;
struct OPS *sequence_of_record_line_opers_ops __CLU_COMMON;
extern struct OPS *equate_ops;
struct OPS *sequence_of_equate_ops __CLU_COMMON;
extern struct OPS *sequence_of_equate_ops;
extern struct OPS *idn_ops;
extern struct OPS *sequence_of_record_line_opers_ops;
struct OPS *record_equates_idn_ops_ops __CLU_COMMON;
extern struct OPS *record_equates_idn_ops_ops;
struct OPS *oneof_has__idn_set_ops __CLU_COMMON;
extern struct OPS *oneof_has__idn_set_ops;
struct OPS *record_idn_kind_line_ops __CLU_COMMON;
extern const struct REQS * const p_clist_of_t_reqs;
extern struct OPS *record_idn_kind_line_ops;
extern const OWN_req p_clist_ownreqs;
struct OPS *p_clist_of_record_idn_kind_line_ops __CLU_COMMON;
static CLUREF STR_restriction;
static int p_where_own_init = 0;

errcode
p_where(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    if (p_where_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)expr_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_expr_ops));
        add_selector_info("name", 0, (struct OPS *)string_ops);
        add_selector_info("parms", 1, (struct OPS *)sequence_of_expr_ops);
        add_selector_info("type_", 2, (struct OPS *)typespec_ops);
        find_selector_ops("record", 3, &(record_name_parms_type__ops));
        add_parm_info_type(0, (const struct OPS *)record_name_parms_type__ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_record_name_parms_type__ops));
        add_selector_info("line", 0, (struct OPS *)int_ops);
        add_selector_info("opers", 1, (struct OPS *)sequence_of_record_name_parms_type__ops);
        find_selector_ops("record", 2, &(record_line_opers_ops));
        add_parm_info_type(0, (const struct OPS *)record_line_opers_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_record_line_opers_ops));
        add_parm_info_type(0, (const struct OPS *)equate_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_equate_ops));
        add_selector_info("equates", 0, (struct OPS *)sequence_of_equate_ops);
        add_selector_info("idn", 1, (struct OPS *)idn_ops);
        add_selector_info("ops", 2, (struct OPS *)sequence_of_record_line_opers_ops);
        find_selector_ops("record", 3, &(record_equates_idn_ops_ops));
        add_selector_info("has_", 0, (struct OPS *)sequence_of_record_line_opers_ops);
        add_selector_info("idn", 1, (struct OPS *)idn_ops);
        add_selector_info("set", 2, (struct OPS *)record_equates_idn_ops_ops);
        find_selector_ops("oneof", 3, &(oneof_has__idn_set_ops));
        add_selector_info("idn", 0, (struct OPS *)idn_ops);
        add_selector_info("kind", 1, (struct OPS *)oneof_has__idn_set_ops);
        add_selector_info("line", 2, (struct OPS *)int_ops);
        find_selector_ops("record", 3, &(record_idn_kind_line_ops));
        add_parm_info_type(0, (const struct OPS *)record_idn_kind_line_ops, p_clist_of_t_reqs);
        find_prociter_instance(p_clist, 1, &p_clist_ownreqs, &(p_clist_of_record_idn_kind_line_ops));
        stringOPcons("restriction", CLU_1, CLUREF_make_num(11), &STR_restriction);
        p_where_own_init = 1;
    }
    enter_proc(9);

  LINE(10);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 40: /* where_ */ {

  LINE(11);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(12);
        { /* return */
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        err = proctypeOPnew(CLU_0, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        T_2_1.proc->type_owns = NULL;
        T_2_1.proc->op_owns = NULL;
        T_2_1.proc->proc = p_restrict;
        generic_CLU_proc.type_owns = NULL;
        generic_CLU_proc.op_owns = p_clist_of_record_idn_kind_line_ops->op_owns;
        generic_CLU_proc.proc = p_clist;
        CUR_PROC_VAR.proc = &generic_CLU_proc;
        err = p_clist(e, T_2_1, STR_restriction, &T_2_2);
        if (err != ERR_ok)
            goto ex_0;
        ret_1->num = T_2_2.num;
        }
        signal (ERR_ok);
        }
        break;
    }
    default: {

  LINE(14);
        { /* return */
        {
        CLUREF T_2_1;
        err = sequenceOPnew(&T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        ret_1->num = T_2_1.num;
        }
        signal (ERR_ok);
        }
        break;
    }
    }
    } /* end qtagcase */

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_where ****/



/**** BEGIN PROCEDURE p_restrict ****/

extern errcode p_idn();
extern errcode p_envOPget_line();
extern errcode oneofOPmake_2();
extern errcode oneofOPmake_3();
extern errcode p_typeset();
extern errcode p_envOPassume();
extern errcode oneofOPmake_1();
extern errcode p_operdecllist();
static CLUREF STR_typeset_040after_040IN;
static CLUREF STR_IN_040or_040HAS_040restriction_040after_040idn_040in_040WHERE_040clause;
static int p_restrict_own_init = 0;

errcode
p_restrict(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF id;
    CLUREF line;
    CLUREF kind;
    CLUREF k;
    if (p_restrict_own_init == 0) {
        stringOPcons("typeset after IN", CLU_1, CLUREF_make_num(16), &STR_typeset_040after_040IN);
        stringOPcons("IN or HAS restriction after idn in WHERE clause", CLU_1, CLUREF_make_num(47), &STR_IN_040or_040HAS_040restriction_040after_040idn_040in_040WHERE_040clause);
        p_restrict_own_init = 1;
    }
    enter_proc(20);

  LINE(21);
    {
    CLUREF T_2_1;
    err = p_idn(e, &T_2_1);
    if (err != ERR_ok)
        goto ex_1;
    id.num = T_2_1.num;
    }
    goto end_1;
  ex_1: /* resignal */
    __CLU_EX_HANDLER;
    if (err == ERR_none)
        signal(err);
    else
        goto ex_0;
  end_1:;

  LINE(23);
    {
    CLUREF T_1_1;
    err = p_envOPget_line(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    line.num = T_1_1.num;
    }

  LINE(25);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 17: /* in_ */ {

  LINE(27);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(28);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 16: /* idn */ {
            CLUREF T_2_2;
            T_2_2.num = T_2_1.cell->value;
            k.num = T_2_2.num;

  LINE(30);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(31);
            {
            CLUREF T_3_1;
            CellAlloc(2, k.num, T_3_1);
            kind.num = T_3_1.num;
            }
            break;
        }
        case 20: /* l_curly */ {

  LINE(33);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(34);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            err = p_typeset(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_2;
            CellAlloc(3, T_4_1.num, T_4_2);
            kind.num = T_4_2.num;
            }
            goto end_2;
          ex_2: /* resignal */
            __CLU_EX_HANDLER;
            if (err == ERR_bad)
                signal(err);
            else
                goto ex_0;
          end_2:;
            break;
        }
        default: {

  LINE(37);
            {
            err = p_envOPassume(e, STR_typeset_040after_040IN);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(38);
            { /* signal */
                signal(ERR_bad);
            }
            break;
        }
        }
        } /* end qtagcase */
        break;
    }
    case 15: /* has_ */ {

  LINE(41);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(42);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        err = p_operdecllist(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        CellAlloc(1, T_2_1.num, T_2_2);
        kind.num = T_2_2.num;
        }
        break;
    }
    default: {

  LINE(44);
        {
        err = p_envOPassume(e, STR_IN_040or_040HAS_040restriction_040after_040idn_040in_040WHERE_040clause);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(45);
        { /* signal */
            signal(ERR_bad);
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(47);
    { /* return */
    {
    CLUREF T_1_1;
    RecordAlloc(3, T_1_1);
    T_1_1.vec->data[0] = id.num;
    T_1_1.vec->data[1] = kind.num;
    T_1_1.vec->data[2] = line.num;
    ret_1->num = T_1_1.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_restrict ****/



/**** BEGIN PROCEDURE p_typeset ****/

extern errcode stringOPequal();
extern errcode structOPget_2();
extern errcode boolOPnot();
extern errcode structOPget_1();
extern errcode idnOPget_str();
extern errcode p_slist();
extern errcode p_equate();
extern const struct REQS * const p_slist_of_t_reqs;
extern const OWN_req p_slist_ownreqs;
struct OPS *p_slist_of_equate_ops __CLU_COMMON;
static CLUREF STR_idn_040after_040_173;
static CLUREF STR_or;
static CLUREF STR_idn_040after_040_174_040in_040_173idn_040_174_040idn_040_056_056_056_175;
static CLUREF STR__174_040idn_040in_040_173idn_040_174_040idn_040_056_056_056_175;
static CLUREF STR__174_040idn_040in_040_173idn_040_174_040_056_056_056_175;
static CLUREF STR_idns_040do_040not_040match_040in_040_173idn_040_174_040idn_040_056_056_056_175;
static CLUREF STR_HAS_040restriction_040in_040typeset;
static CLUREF STR__175_040in_040_173idn_040_174_040_056_056_056_175;
static int p_typeset_own_init = 0;

errcode
p_typeset(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF id;
    CLUREF id2;
    CLUREF k;
    CLUREF ops;
    CLUREF equates;
    CLUREF set;
    if (p_typeset_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)equate_ops, p_slist_of_t_reqs);
        find_prociter_instance(p_slist, 1, &p_slist_ownreqs, &(p_slist_of_equate_ops));
        stringOPcons("idn after {", CLU_1, CLUREF_make_num(11), &STR_idn_040after_040_173);
        stringOPcons("or", CLU_1, CLUREF_make_num(2), &STR_or);
        stringOPcons("idn after | in {idn | idn ...}", CLU_1, CLUREF_make_num(30), &STR_idn_040after_040_174_040in_040_173idn_040_174_040idn_040_056_056_056_175);
        stringOPcons("| idn in {idn | idn ...}", CLU_1, CLUREF_make_num(24), &STR__174_040idn_040in_040_173idn_040_174_040idn_040_056_056_056_175);
        stringOPcons("| idn in {idn | ...}", CLU_1, CLUREF_make_num(20), &STR__174_040idn_040in_040_173idn_040_174_040_056_056_056_175);
        stringOPcons("idns do not match in {idn | idn ...}", CLU_1, CLUREF_make_num(36), &STR_idns_040do_040not_040match_040in_040_173idn_040_174_040idn_040_056_056_056_175);
        stringOPcons("HAS restriction in typeset", CLU_1, CLUREF_make_num(26), &STR_HAS_040restriction_040in_040typeset);
        stringOPcons("} in {idn | ...}", CLU_1, CLUREF_make_num(16), &STR__175_040in_040_173idn_040_174_040_056_056_056_175);
        p_typeset_own_init = 1;
    }
    enter_proc(54);

  LINE(55);
    {
    CLUREF T_2_1;
    err = p_idn(e, &T_2_1);
    if (err != ERR_ok)
        goto ex_1;
    id.num = T_2_1.num;
    }
    goto end_1;
  ex_1: /* except */
    __CLU_EX_HANDLER;
    if (err == ERR_none) {

  LINE(56);
        {
        err = p_envOPassume(e, STR_idn_040after_040_173);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(57);
        { /* signal */
            signal(ERR_bad);
        }
    }
    else { /* not handled */
        goto ex_0;
    }
  end_1:;

  LINE(59);
    {
    id2.num = id.num;
    }

  LINE(60);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 23: /* op */ {
        CLUREF T_1_2;
        T_1_2.num = T_1_1.cell->value;
        k.num = T_1_2.num;

  LINE(62);
        { /* if */
        CLUREF T_2_1;
        CLUREF T_2_2;
        CLUREF T_2_3;
        CLUREF T_2_4;
        CLUREF T_2_5;
        T_2_2.num = k.vec->data[1];
        T_2_3.num = ((STR_or.str->size != T_2_2.str->size)? false :
            !(memcmp(STR_or.str->data, T_2_2.str->data, STR_or.str->size)));
        T_2_1.num = T_2_3.num;
        if (T_2_3.num) {
            T_2_4.num = k.vec->data[0];
            T_2_5.num = !T_2_4.num;
            T_2_1.num = T_2_5.num;
        }
        if (T_2_1.num == true) { /* if */

  LINE(63);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(64);
            {
            CLUREF T_4_1;
            err = p_idn(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_2;
            id2.num = T_4_1.num;
            }
            goto end_2;
          ex_2: /* except */
            __CLU_EX_HANDLER;
            if (err == ERR_none) {

  LINE(65);
                {
                err = p_envOPassume(e, STR_idn_040after_040_174_040in_040_173idn_040_174_040idn_040_056_056_056_175);
                if (err != ERR_ok)
                    goto ex_0;
                }
            }
            else { /* not handled */
                goto ex_0;
            }
          end_2:;
        }
        else { /* else */

  LINE(68);
            {
            err = p_envOPassume(e, STR__174_040idn_040in_040_173idn_040_174_040idn_040_056_056_056_175);
            if (err != ERR_ok)
                goto ex_0;
            }
        }} /* end if */
        break;
    }
    default: {

  LINE(71);
        {
        err = p_envOPassume(e, STR__174_040idn_040in_040_173idn_040_174_040_056_056_056_175);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(73);
    { /* if */
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
    CLUREF T_1_4;
    err = idnOPget_str(id, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    err = idnOPget_str(id2, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    T_1_3.num = ((T_1_1.str->size != T_1_2.str->size)? false :
        !(memcmp(T_1_1.str->data, T_1_2.str->data, T_1_1.str->size)));
    T_1_4.num = !T_1_3.num;
    if (T_1_4.num == true) { /* if */

  LINE(74);
        {
        err = p_envOPassume(e, STR_idns_040do_040not_040match_040in_040_173idn_040_174_040idn_040_056_056_056_175);
        if (err != ERR_ok)
            goto ex_0;
        }
    }
    } /* end if */

  LINE(76);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 15: /* has_ */ {

  LINE(77);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(78);
        {
        CLUREF T_2_1;
        err = p_operdecllist(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        ops.num = T_2_1.num;
        }
        break;
    }
    default: {

  LINE(79);
        {
        err = p_envOPassume(e, STR_HAS_040restriction_040in_040typeset);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(80);
        {
        CLUREF T_2_1;
        err = sequenceOPnew(&T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        ops.num = T_2_1.num;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(82);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = proctypeOPnew(CLU_0, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.proc->type_owns = NULL;
    T_1_1.proc->op_owns = NULL;
    T_1_1.proc->proc = p_equate;
    generic_CLU_proc.type_owns = NULL;
    generic_CLU_proc.op_owns = p_slist_of_equate_ops->op_owns;
    generic_CLU_proc.proc = p_slist;
    CUR_PROC_VAR.proc = &generic_CLU_proc;
    err = p_slist(e, T_1_1, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    equates.num = T_1_2.num;
    }

  LINE(83);
    {
    CLUREF T_1_1;
    RecordAlloc(3, T_1_1);
    T_1_1.vec->data[1] = id.num;
    T_1_1.vec->data[2] = ops.num;
    T_1_1.vec->data[0] = equates.num;
    set.num = T_1_1.num;
    }

  LINE(86);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 27: /* r_curly */ {

  LINE(87);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    default: {

  LINE(88);
        {
        err = p_envOPassume(e, STR__175_040in_040_173idn_040_174_040_056_056_056_175);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(90);
    { /* return */
    {
    ret_1->num = set.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_typeset ****/



/**** BEGIN PROCEDURE p_operdecllist ****/

extern errcode p_envOPget_peekline();
extern errcode p_opname();
extern errcode p_type1();
extern errcode p_envOPget_unknown_type();
extern errcode sequenceOPelements();
extern errcode recordOPset_3();
extern errcode sequenceOPaddh();
extern errcode p_envOPpeek();
struct OPS *sequence_of_expr_ops __CLU_COMMON;
struct OPS *record_name_parms_type__ops __CLU_COMMON;
struct OPS *p_clist_of_record_name_parms_type__ops __CLU_COMMON;
static CLUREF STR_opname;
static CLUREF STR__072_040type_040of_040operation_040declaration_040in_040type_040restriction;
static int p_operdecllist_own_init = 0;

errcode
p_operdecllist(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF list;
    CLUREF line;
    CLUREF opers;
    CLUREF typ;
    CLUREF op;
    if (p_operdecllist_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)expr_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_expr_ops));
        add_selector_info("name", 0, (struct OPS *)string_ops);
        add_selector_info("parms", 1, (struct OPS *)sequence_of_expr_ops);
        add_selector_info("type_", 2, (struct OPS *)typespec_ops);
        find_selector_ops("record", 3, &(record_name_parms_type__ops));
        add_parm_info_type(0, (const struct OPS *)record_name_parms_type__ops, p_clist_of_t_reqs);
        find_prociter_instance(p_clist, 1, &p_clist_ownreqs, &(p_clist_of_record_name_parms_type__ops));
        stringOPcons("opname", CLU_1, CLUREF_make_num(6), &STR_opname);
        stringOPcons(": type of operation declaration in type restriction", CLU_1, CLUREF_make_num(51), &STR__072_040type_040of_040operation_040declaration_040in_040type_040restriction);
        p_operdecllist_own_init = 1;
    }
    enter_proc(96);

  LINE(97);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    list.num = T_1_1.num;
    }

  LINE(98);
    for (;;) { /* while */
        if (true != true)
            break;

  LINE(99);
        {
        CLUREF T_2_1;
        err = p_envOPget_peekline(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        line.num = T_2_1.num;
        }

  LINE(100);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        err = proctypeOPnew(CLU_0, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        T_2_1.proc->type_owns = NULL;
        T_2_1.proc->op_owns = NULL;
        T_2_1.proc->proc = p_opname;
        generic_CLU_proc.type_owns = NULL;
        generic_CLU_proc.op_owns = p_clist_of_record_name_parms_type__ops->op_owns;
        generic_CLU_proc.proc = p_clist;
        CUR_PROC_VAR.proc = &generic_CLU_proc;
        err = p_clist(e, T_2_1, STR_opname, &T_2_2);
        if (err != ERR_ok)
            goto ex_0;
        opers.num = T_2_2.num;
        }

  LINE(102);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 2: /* colon */ {

  LINE(103);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(104);
            {
            CLUREF T_3_1;
            err = p_type1(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            typ.num = T_3_1.num;
            }
            break;
        }
        default: {

  LINE(105);
            {
            err = p_envOPassume(e, STR__072_040type_040of_040operation_040declaration_040in_040type_040restriction);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(107);
            {
            CLUREF T_3_1;
            err = p_envOPget_unknown_type(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            typ.num = T_3_1.num;
            }
            break;
        }
        }
        } /* end qtagcase */

  LINE(109);
        { /* for sequence$elements */
        CLUREF T_2_1;
        CLUREF T_2_2;
        CLUREF T_2_3;
        T_2_2.num = opers.vec->size;
        T_2_3 = opers;
        for (T_2_1.num = 1; T_2_1.num <= T_2_2.num; T_2_1.num++) {
            op.num = T_2_3.vec->data[T_2_1.num - 1];

  LINE(110);
            {
            op.vec->data[2] = typ.num;
            }
        }
        }
        end_inline_for_1:
        __CLU_END_LABEL;

  LINE(112);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        RecordAlloc(2, T_2_1);
        T_2_1.vec->data[1] = opers.num;
        T_2_1.vec->data[0] = line.num;
        err = sequenceOPaddh(list, T_2_1, &T_2_2);
        if (err != ERR_ok)
            goto ex_0;
        list.num = T_2_2.num;
        }

  LINE(114);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 3: /* comma */ {

  LINE(115);
            { /* qtagcase */
            CLUREF T_3_1;
            err = p_envOPpeek(e, CLU_3, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            switch (T_3_1.cell->tag) {
            case 15: /* has_ */ /* FALLTHROUGH */
            case 17: /* in_ */ {

  LINE(116);
                { /* return */
                {
                ret_1->num = list.num;
                }
                signal (ERR_ok);
                }
                break;
            }
            default: {

  LINE(117);
                {
                CLUREF T_4_1;
                err = p_envOPnext_token(e, &T_4_1);
                if (err != ERR_ok)
                    goto ex_0;
                }
                break;
            }
            }
            } /* end qtagcase */
            break;
        }
        default: {

  LINE(119);
            { /* return */
            {
            ret_1->num = list.num;
            }
            signal (ERR_ok);
            }
            break;
        }
        }
        } /* end qtagcase */
    }
    end_while_1:
    __CLU_END_LABEL;

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_operdecllist ****/



/**** BEGIN PROCEDURE p_opname ****/

extern errcode p_name();
extern errcode p_blist();
extern errcode p_expr();
extern const struct REQS * const p_blist_of_t_reqs;
extern const OWN_req p_blist_ownreqs;
struct OPS *p_blist_of_expr_ops __CLU_COMMON;
static CLUREF STR_constant;
static int p_opname_own_init = 0;

errcode
p_opname(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF n;
    CLUREF parms;
    if (p_opname_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)expr_ops, p_blist_of_t_reqs);
        find_prociter_instance(p_blist, 1, &p_blist_ownreqs, &(p_blist_of_expr_ops));
        stringOPcons("constant", CLU_1, CLUREF_make_num(8), &STR_constant);
        p_opname_own_init = 1;
    }
    enter_proc(128);

  LINE(129);
    {
    CLUREF T_2_1;
    err = p_name(e, &T_2_1);
    if (err != ERR_ok)
        goto ex_1;
    n.num = T_2_1.num;
    }
    goto end_1;
  ex_1: /* resignal */
    __CLU_EX_HANDLER;
    if (err == ERR_none)
        signal(err);
    else
        goto ex_0;
  end_1:;

  LINE(131);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = proctypeOPnew(CLU_0, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.proc->type_owns = NULL;
    T_1_1.proc->op_owns = NULL;
    T_1_1.proc->proc = p_expr;
    generic_CLU_proc.type_owns = NULL;
    generic_CLU_proc.op_owns = p_blist_of_expr_ops->op_owns;
    generic_CLU_proc.proc = p_blist;
    CUR_PROC_VAR.proc = &generic_CLU_proc;
    err = p_blist(e, T_1_1, CLU_true, STR_constant, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    parms.num = T_1_2.num;
    }

  LINE(132);
    { /* return */
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    RecordAlloc(3, T_1_1);
    T_1_1.vec->data[0] = n.num;
    T_1_1.vec->data[1] = parms.num;
    err = p_envOPget_unknown_type(e, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.vec->data[2] = T_1_2.num;
    ret_1->num = T_1_1.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_opname ****/



/**** BEGIN PROCEDURE p_pathname ****/

extern errcode oneofOPvalue_24();
extern errcode oneofOPvalue_13();
static CLUREF STR_pathname_040in_040idn_040_075_040PATHNAME_040pathname;
static int p_pathname_own_init = 0;

errcode
p_pathname(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF s;
    if (p_pathname_own_init == 0) {
        stringOPcons("pathname in idn = PATHNAME pathname", CLU_1, CLUREF_make_num(35), &STR_pathname_040in_040idn_040_075_040PATHNAME_040pathname);
        p_pathname_own_init = 1;
    }
    enter_proc(139);

  LINE(140);
    { /* begin */

  LINE(141);
        {
        CLUREF T_3_1;
        CLUREF T_3_2;
        CLUREF T_3_3;
        err = p_envOPget_peek1(e, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        if (T_3_1.cell->tag != 13) {
            err = ERR_wrong_tag;
            goto ex_1;
        }
        T_3_2.num = T_3_1.cell->value;
        if (T_3_2.cell->tag != 24) {
            err = ERR_wrong_tag;
            goto ex_1;
        }
        T_3_3.num = T_3_2.cell->value;
        s.num = T_3_3.num;
        }

  LINE(142);
        {
        CLUREF T_3_1;
        err = p_envOPnext_token(e, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        }

  LINE(143);
        { /* return */
        {
        ret_1->num = s.num;
        }
        signal (ERR_ok);
        }
    }
    goto end_1;
  ex_1: /* except */
    __CLU_EX_HANDLER;
    if (err == ERR_wrong_tag) {
    }
    else { /* not handled */
        goto ex_0;
    }
  end_1:;

  LINE(145);
    {
    err = p_envOPassume(e, STR_pathname_040in_040idn_040_075_040PATHNAME_040pathname);
    if (err != ERR_ok)
        goto ex_0;
    }

  LINE(146);
    { /* signal */
        signal(ERR_bad);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_pathname ****/



/**** BEGIN PROCEDURE p_defn_end ****/

extern errcode stringOPconcat();
extern errcode oneofOPvalue_16();
extern errcode p_envOPget_clustr();
extern errcode p_envOPerr();
extern errcode p_semi();
static CLUREF STR_END_040and_040trailing_040idn_040of_040module_040;
static CLUREF STR_trailing_040idn_040does_040not_040match_040;
static CLUREF STR_trailing_040idn_040of_040module_040;
static int p_defn_end_own_init = 0;

errcode
p_defn_end(CLUREF e, CLUREF id)
{
    errcode err;
    CLUREF s;
    if (p_defn_end_own_init == 0) {
        stringOPcons("END and trailing idn of module ", CLU_1, CLUREF_make_num(31), &STR_END_040and_040trailing_040idn_040of_040module_040);
        stringOPcons("trailing idn does not match ", CLU_1, CLUREF_make_num(28), &STR_trailing_040idn_040does_040not_040match_040);
        stringOPcons("trailing idn of module ", CLU_1, CLUREF_make_num(23), &STR_trailing_040idn_040of_040module_040);
        p_defn_end_own_init = 1;
    }
    enter_proc(151);

  LINE(152);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 9: /* end_ */ {
        break;
    }
    default: {

  LINE(154);
        {
        CLUREF T_2_1;
        err = stringOPconcat(STR_END_040and_040trailing_040idn_040of_040module_040, id, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        err = p_envOPassume(e, T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(155);
        { /* return */
        signal (ERR_ok);
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(157);
    { /* begin */

  LINE(158);
        {
        CLUREF T_3_1;
        CLUREF T_3_2;
        CLUREF T_3_3;
        err = p_envOPpeek(e, CLU_2, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        if (T_3_1.cell->tag != 16) {
            err = ERR_wrong_tag;
            goto ex_1;
        }
        T_3_2.num = T_3_1.cell->value;
        err = idnOPget_str(T_3_2, &T_3_3);
        if (err != ERR_ok)
            goto ex_1;
        s.num = T_3_3.num;
        }

  LINE(159);
        { /* if */
        CLUREF T_3_1;
        T_3_1.num = ((id.str->size != s.str->size)? false :
            !(memcmp(id.str->data, s.str->data, id.str->size)));
        if (T_3_1.num == true) { /* if */

  LINE(160);
            {
            CLUREF T_4_1;
            err = p_envOPnext_token(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            }

  LINE(161);
            {
            CLUREF T_4_1;
            err = p_envOPnext_token(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            }
        }
        else {

  LINE(162);
        CLUREF T_3_2;
        CLUREF T_3_3;
        err = p_envOPget_clustr(e, &T_3_2);
        if (err != ERR_ok)
            goto ex_1;
        T_3_3.num = ((s.str->size != T_3_2.str->size)? false :
            !(memcmp(s.str->data, T_3_2.str->data, s.str->size)));
        if (T_3_3.num == true) { /* elseif */

  LINE(163);
            {
            CLUREF T_4_1;
            err = stringOPconcat(STR_END_040and_040trailing_040idn_040of_040module_040, id, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            err = p_envOPassume(e, T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            }
        }
        else { /* else */

  LINE(164);
            {
            CLUREF T_4_1;
            err = p_envOPnext_token(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            }

  LINE(165);
            { /* qtagcase */
            CLUREF T_4_1;
            err = p_envOPpeek(e, CLU_2, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            switch (T_4_1.cell->tag) {
            case 31: /* semi */ /* FALLTHROUGH */
            case 9: /* end_ */ /* FALLTHROUGH */
            case 16: /* idn */ /* FALLTHROUGH */
            case 10: /* eof */ {

  LINE(167);
                {
                CLUREF T_5_1;
                err = p_envOPnext_token(e, &T_5_1);
                if (err != ERR_ok)
                    goto ex_1;
                }

  LINE(168);
                {
                CLUREF T_5_1;
                err = stringOPconcat(STR_trailing_040idn_040does_040not_040match_040, id, &T_5_1);
                if (err != ERR_ok)
                    goto ex_1;
                err = p_envOPerr(e, T_5_1, CLU_1);
                if (err != ERR_ok)
                    goto ex_1;
                }
                break;
            }
            default: {

  LINE(170);
                {
                CLUREF T_5_1;
                err = stringOPconcat(STR_trailing_040idn_040of_040module_040, id, &T_5_1);
                if (err != ERR_ok)
                    goto ex_1;
                err = p_envOPassume(e, T_5_1);
                if (err != ERR_ok)
                    goto ex_1;
                }
                break;
            }
            }
            } /* end qtagcase */
        }}} /* end if */
    }
    goto end_1;
  ex_1: /* except */
    __CLU_EX_HANDLER;
    if (err == ERR_wrong_tag) {

  LINE(174);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(175);
        {
        CLUREF T_2_1;
        err = stringOPconcat(STR_trailing_040idn_040of_040module_040, id, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        err = p_envOPassume(e, T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }
    }
    else { /* not handled */
        goto ex_0;
    }
  end_1:;

  LINE(177);
    {
    err = p_semi(e);
    if (err != ERR_ok)
        goto ex_0;
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    signal(ERR_ok);
}

/**** END PROCEDURE p_defn_end ****/



/**** BEGIN PROCEDURE p_semi ****/

extern errcode intOPadd();
extern errcode intOPgt();
static CLUREF STR_more_040than_040one_040_073_040in_040a_040row;
static int p_semi_own_init = 0;

errcode
p_semi(CLUREF e)
{
    errcode err;
    CLUREF i;
    if (p_semi_own_init == 0) {
        stringOPcons("more than one ; in a row", CLU_1, CLUREF_make_num(24), &STR_more_040than_040one_040_073_040in_040a_040row);
        p_semi_own_init = 1;
    }
    enter_proc(182);

  LINE(183);
    {
    i.num = 0;
    }

  LINE(184);
    for (;;) { /* while */
        if (true != true)
            break;

  LINE(185);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 31: /* semi */ {

  LINE(187);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(188);
            {
            CLUREF T_3_1;
            T_3_1.num = i.num + 1;
            if ((T_3_1.num > 0 && i.num < 0 && 1 < 0) ||
                (T_3_1.num < 0 && i.num > 0 && 1 > 0)) {
                err = ERR_overflow;
                goto ex_0;
            }
            i.num = T_3_1.num;
            }
            break;
        }
        default: {

  LINE(190);
            { /* if */
            CLUREF T_3_1;
            T_3_1.num = (i.num > 1);
            if (T_3_1.num == true) { /* if */

  LINE(191);
                {
                err = p_envOPerr(e, STR_more_040than_040one_040_073_040in_040a_040row, CLU_1);
                if (err != ERR_ok)
                    goto ex_0;
                }
            }
            } /* end if */

  LINE(193);
            { /* return */
            signal (ERR_ok);
            }
            break;
        }
        }
        } /* end qtagcase */
    }
    end_while_1:
    __CLU_END_LABEL;

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    signal(ERR_ok);
}

/**** END PROCEDURE p_semi ****/



/**** BEGIN PROCEDURE p_find_a_defn ****/

extern errcode intOPequal();
static CLUREF STR_looking_040for_040an_040operation;
static CLUREF STR_looking_040for_040a_040module;
static CLUREF STR_resuming_040parse_040at_040this_040point;
static int p_find_a_defn_own_init = 0;

errcode
p_find_a_defn(CLUREF e, CLUREF want_oper, CLUREF *ret_1)
{
    errcode err;
    CLUREF flushed;
    CLUREF found;
    CLUREF line;
    if (p_find_a_defn_own_init == 0) {
        stringOPcons("looking for an operation", CLU_1, CLUREF_make_num(24), &STR_looking_040for_040an_040operation);
        stringOPcons("looking for a module", CLU_1, CLUREF_make_num(20), &STR_looking_040for_040a_040module);
        stringOPcons("resuming parse at this point", CLU_1, CLUREF_make_num(28), &STR_resuming_040parse_040at_040this_040point);
        p_find_a_defn_own_init = 1;
    }
    enter_proc(201);

  LINE(202);
    {
    flushed.tf = false;
    }

  LINE(203);
    {
    found.tf = false;
    }

  LINE(204);
    for (;;) { /* while */
        if (true != true)
            break;

  LINE(205);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 16: /* idn */ {

  LINE(207);
            { /* qtagcase */
            CLUREF T_3_1;
            err = p_envOPpeek(e, CLU_2, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            switch (T_3_1.cell->tag) {
            case 11: /* equal */ {

  LINE(208);
                {
                found.tf = true;
                }

  LINE(209);
                goto end_while_1;
                break;
            }
            }
            } /* end qtagcase */
            break;
        }
        case 9: /* end_ */ {

  LINE(213);
            { /* if */
            if (want_oper.num == true) { /* if */
                goto end_while_1;
            }
            } /* end if */
            break;
        }
        case 10: /* eof */ {

  LINE(215);
            goto end_while_1;
            break;
        }
        }
        } /* end qtagcase */

  LINE(218);
        { /* if */
        CLUREF T_2_1;
        T_2_1.num = !flushed.num;
        if (T_2_1.num == true) { /* if */

  LINE(219);
            { /* if */
            if (want_oper.num == true) { /* if */

  LINE(220);
                {
                err = p_envOPerr(e, STR_looking_040for_040an_040operation, CLU_2);
                if (err != ERR_ok)
                    goto ex_0;
                }
            }
            else { /* else */

  LINE(221);
                {
                err = p_envOPerr(e, STR_looking_040for_040a_040module, CLU_2);
                if (err != ERR_ok)
                    goto ex_0;
                }
            }} /* end if */

  LINE(223);
            {
            flushed.tf = true;
            }
        }
        } /* end if */

  LINE(225);
        {
        CLUREF T_2_1;
        err = p_envOPget_peekline(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        line.num = T_2_1.num;
        }

  LINE(226);
        for (;;) { /* while */
            CLUREF T_2_1;
            CLUREF T_2_2;
            err = p_envOPget_peekline(e, &T_2_1);
            if (err != ERR_ok)
                goto ex_0;
            T_2_2.num = (T_2_1.num == line.num);
            if (T_2_2.num != true)
                break;

  LINE(227);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }
        }
        end_while_2:
        __CLU_END_LABEL;
    }
    end_while_1:
    __CLU_END_LABEL;

  LINE(230);
    { /* if */
    if (flushed.num == true) { /* if */

  LINE(231);
        {
        err = p_envOPerr(e, STR_resuming_040parse_040at_040this_040point, CLU_0);
        if (err != ERR_ok)
            goto ex_0;
        }
    }
    } /* end if */

  LINE(232);
    { /* return */
    {
    ret_1->num = found.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_find_a_defn ****/

