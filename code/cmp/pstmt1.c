
/* This file was automatically generated by pclu. */

#include "pclu_err.h"
#include "pclu_sys.h"



/**** BEGIN PROCEDURE p_stmt ****/

extern errcode p_envOPget_peekline();
extern errcode p_envOPget_peek1();
extern errcode p_envOPnext_token();
extern errcode p_if();
extern errcode p_for();
extern errcode p_tagcase();
extern errcode p_while();
extern errcode boolOPnot();
extern errcode p_envOPin_proc();
extern errcode p_envOPspecd_vals();
extern errcode oneofOPmake_15();
extern errcode p_plist();
extern errcode p_expr();
extern errcode oneofOPmake_20();
extern errcode oneofOPmake_16();
extern errcode p_condition();
extern errcode oneofOPmake_9();
extern errcode oneofOPmake_4();
extern errcode oneofOPmake_5();
extern errcode oneofOPmake_3();
extern errcode p_body();
extern errcode p_end();
extern errcode p_ownstmt();
extern errcode p_envOPpeek();
extern errcode p_decl_or_assn();
extern errcode p_expr_stmt();
extern errcode p_forcestmt();
extern errcode p_envOPerr();
extern errcode oneofOPmake_2();
extern errcode p_find_a_stmt();
extern errcode p_except();
extern errcode stmtOPcreate();
extern const struct REQS * const p_plist_of_t_reqs;
extern struct OPS *expr_ops;
extern const OWN_req p_plist_ownreqs;
struct OPS *p_plist_of_expr_ops __CLU_COMMON;
static CLUREF STR_expression;
static CLUREF STR_END_040of_040BEGIN_055END_040statement;
static CLUREF STR_operator_040or_040_050_040starts_040statement;
static CLUREF STR_EXCEPT_040clause_040not_040attached_040to_040statement;
static int p_stmt_own_init = 0;

errcode
p_stmt(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF abs;
    CLUREF l;
    CLUREF k;
    CLUREF opt;
    if (p_stmt_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)expr_ops, p_plist_of_t_reqs);
        find_prociter_instance(p_plist, 1, &p_plist_ownreqs, &(p_plist_of_expr_ops));
        stringOPcons("expression", CLU_1, CLUREF_make_num(10), &STR_expression);
        stringOPcons("END of BEGIN-END statement", CLU_1, CLUREF_make_num(26), &STR_END_040of_040BEGIN_055END_040statement);
        stringOPcons("operator or ( starts statement", CLU_1, CLUREF_make_num(30), &STR_operator_040or_040_050_040starts_040statement);
        stringOPcons("EXCEPT clause not attached to statement", CLU_1, CLUREF_make_num(39), &STR_EXCEPT_040clause_040not_040attached_040to_040statement);
        p_stmt_own_init = 1;
    }
    enter_proc(12);

  LINE(14);
    {
    CLUREF T_1_1;
    err = p_envOPget_peekline(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    l.num = T_1_1.num;
    }

  LINE(15);
    { /* qtagcase */
    CLUREF T_2_1;
    err = p_envOPget_peek1(e, &T_2_1);
    if (err != ERR_ok)
        goto ex_1;
    switch (T_2_1.cell->tag) {
    case 34: /* stmtkey */ {
        CLUREF T_2_2;
        T_2_2.num = T_2_1.cell->value;
        k.num = T_2_2.num;

  LINE(17);
        {
        CLUREF T_3_1;
        err = p_envOPnext_token(e, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        }

  LINE(18);
        { /* qtagcase */
        switch (k.cell->tag) {
        case 6: /* if_ */ {

  LINE(19);
            {
            CLUREF T_4_1;
            err = p_if(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        case 5: /* for_ */ {

  LINE(20);
            {
            CLUREF T_4_1;
            err = p_for(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        case 10: /* tagcase_ */ {

  LINE(21);
            {
            CLUREF T_4_1;
            err = p_tagcase(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        case 11: /* while_ */ {

  LINE(22);
            {
            CLUREF T_4_1;
            err = p_while(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        case 8: /* return_ */ {

  LINE(23);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            CLUREF T_4_3;
            CLUREF T_4_4;
            err = p_envOPin_proc(e, &T_4_2);
            if (err != ERR_ok)
                goto ex_1;
            T_4_1.num = T_4_2.num;
            if (T_4_2.num) {
                err = p_envOPspecd_vals(e, &T_4_3);
                if (err != ERR_ok)
                    goto ex_1;
                T_4_1.num = T_4_3.num;
            }
            T_4_4.num = !T_4_1.num;
            opt.num = T_4_4.num;
            }

  LINE(25);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            CLUREF T_4_3;
            err = proctypeOPnew(CLU_0, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            T_4_1.proc->type_owns = NULL;
            T_4_1.proc->op_owns = NULL;
            T_4_1.proc->proc = p_expr;
            generic_CLU_proc.type_owns = NULL;
            generic_CLU_proc.op_owns = p_plist_of_expr_ops->op_owns;
            generic_CLU_proc.proc = p_plist;
            CUR_PROC_VAR.proc = &generic_CLU_proc;
            err = p_plist(e, T_4_1, opt, CLU_false, STR_expression, &T_4_2);
            if (err != ERR_ok)
                goto ex_1;
            CellAlloc(15, T_4_2.num, T_4_3);
            abs.num = T_4_3.num;
            }
            break;
        }
        case 12: /* yield_ */ {

  LINE(28);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            CLUREF T_4_3;
            CLUREF T_4_4;
            err = p_envOPin_proc(e, &T_4_2);
            if (err != ERR_ok)
                goto ex_1;
            T_4_1.num = T_4_2.num;
            if (!T_4_2.num) {
                err = p_envOPspecd_vals(e, &T_4_3);
                if (err != ERR_ok)
                    goto ex_1;
                T_4_4.num = !T_4_3.num;
                T_4_1.num = T_4_4.num;
            }
            opt.num = T_4_1.num;
            }

  LINE(30);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            CLUREF T_4_3;
            err = proctypeOPnew(CLU_0, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            T_4_1.proc->type_owns = NULL;
            T_4_1.proc->op_owns = NULL;
            T_4_1.proc->proc = p_expr;
            generic_CLU_proc.type_owns = NULL;
            generic_CLU_proc.op_owns = p_plist_of_expr_ops->op_owns;
            generic_CLU_proc.proc = p_plist;
            CUR_PROC_VAR.proc = &generic_CLU_proc;
            err = p_plist(e, T_4_1, opt, CLU_false, STR_expression, &T_4_2);
            if (err != ERR_ok)
                goto ex_1;
            CellAlloc(20, T_4_2.num, T_4_3);
            abs.num = T_4_3.num;
            }
            break;
        }
        case 9: /* signal_ */ {

  LINE(33);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            err = p_condition(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            CellAlloc(16, T_4_1.num, T_4_2);
            abs.num = T_4_2.num;
            }
            break;
        }
        case 4: /* exit_ */ {

  LINE(34);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            err = p_condition(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            CellAlloc(9, T_4_1.num, T_4_2);
            abs.num = T_4_2.num;
            }
            break;
        }
        case 2: /* break_ */ {

  LINE(35);
            {
            CLUREF T_4_1;
            CellAlloc(4, nil, T_4_1);
            abs.num = T_4_1.num;
            }
            break;
        }
        case 3: /* continue_ */ {

  LINE(36);
            {
            CLUREF T_4_1;
            CellAlloc(5, nil, T_4_1);
            abs.num = T_4_1.num;
            }
            break;
        }
        case 1: /* begin_ */ {

  LINE(37);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            err = p_body(e, CLU_3, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            CellAlloc(3, T_4_1.num, T_4_2);
            abs.num = T_4_2.num;
            }

  LINE(38);
            {
            err = p_end(e, STR_END_040of_040BEGIN_055END_040statement);
            if (err != ERR_ok)
                goto ex_1;
            }
            break;
        }
        case 7: /* own_ */ {

  LINE(39);
            {
            CLUREF T_4_1;
            err = p_ownstmt(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        }
        } /* end qtagcase */
        break;
    }
    case 16: /* idn */ {

  LINE(42);
        { /* qtagcase */
        CLUREF T_3_1;
        err = p_envOPpeek(e, CLU_2, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        switch (T_3_1.cell->tag) {
        case 3: /* comma */ /* FALLTHROUGH */
        case 1: /* assn */ /* FALLTHROUGH */
        case 2: /* colon */ {

  LINE(44);
            {
            CLUREF T_4_1;
            err = p_decl_or_assn(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        case 11: /* equal */ {

  LINE(46);
            { /* qtagcase */
            CLUREF T_4_1;
            err = p_envOPpeek(e, CLU_3, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            switch (T_4_1.cell->tag) {
            case 4: /* defnkey */ {

  LINE(47);
                { /* signal */
                    signal(ERR_none);
                }
                break;
            }
            default: {

  LINE(48);
                {
                CLUREF T_5_1;
                err = p_decl_or_assn(e, &T_5_1);
                if (err != ERR_ok)
                    goto ex_1;
                abs.num = T_5_1.num;
                }
                break;
            }
            }
            } /* end qtagcase */
            break;
        }
        default: {

  LINE(51);
            {
            CLUREF T_4_1;
            err = p_expr_stmt(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        }
        } /* end qtagcase */
        break;
    }
    case 14: /* exprkey */ {
        CLUREF T_2_3;
        T_2_3.num = T_2_1.cell->value;
        k.num = T_2_3.num;

  LINE(54);
        { /* qtagcase */
        switch (k.cell->tag) {
        case 2: /* force_ */ {

  LINE(56);
            { /* qtagcase */
            CLUREF T_4_1;
            err = p_envOPpeek(e, CLU_2, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            switch (T_4_1.cell->tag) {
            case 16: /* idn */ {

  LINE(57);
                {
                CLUREF T_5_1;
                err = p_forcestmt(e, &T_5_1);
                if (err != ERR_ok)
                    goto ex_1;
                abs.num = T_5_1.num;
                }
                break;
            }
            default: {

  LINE(58);
                {
                CLUREF T_5_1;
                err = p_expr_stmt(e, &T_5_1);
                if (err != ERR_ok)
                    goto ex_1;
                abs.num = T_5_1.num;
                }
                break;
            }
            }
            } /* end qtagcase */
            break;
        }
        default: {

  LINE(61);
            {
            CLUREF T_4_1;
            err = p_expr_stmt(e, &T_4_1);
            if (err != ERR_ok)
                goto ex_1;
            abs.num = T_4_1.num;
            }
            break;
        }
        }
        } /* end qtagcase */
        break;
    }
    case 37: /* type_ */ /* FALLTHROUGH */
    case 38: /* typekey */ /* FALLTHROUGH */
    case 13: /* expr */ {

  LINE(64);
        {
        CLUREF T_3_1;
        err = p_expr_stmt(e, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        abs.num = T_3_1.num;
        }
        break;
    }
    case 23: /* op */ /* FALLTHROUGH */
    case 22: /* not */ /* FALLTHROUGH */
    case 21: /* l_paren */ {

  LINE(66);
        {
        err = p_envOPerr(e, STR_operator_040or_040_050_040starts_040statement, CLU_1);
        if (err != ERR_ok)
            goto ex_1;
        }

  LINE(67);
        {
        CLUREF T_3_1;
        err = p_expr_stmt(e, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        abs.num = T_3_1.num;
        }
        break;
    }
    case 12: /* except_ */ {

  LINE(69);
        {
        err = p_envOPerr(e, STR_EXCEPT_040clause_040not_040attached_040to_040statement, CLU_1);
        if (err != ERR_ok)
            goto ex_1;
        }

  LINE(70);
        {
        CLUREF T_3_1;
        CellAlloc(2, nil, T_3_1);
        abs.num = T_3_1.num;
        }
        break;
    }
    default: {

  LINE(72);
        { /* if */
        CLUREF T_3_1;
        err = p_find_a_stmt(e, &T_3_1);
        if (err != ERR_ok)
            goto ex_1;
        if (T_3_1.num == true) { /* if */

  LINE(73);
            {
            CLUREF T_4_1;
            CellAlloc(2, nil, T_4_1);
            abs.num = T_4_1.num;
            }
        }
        else { /* else */

  LINE(74);
            { /* signal */
                signal(ERR_none);
            }
        }} /* end if */
        break;
    }
    }
    } /* end qtagcase */
    goto end_1;
  ex_1: /* except */
    __CLU_EX_HANDLER;
    if (err == ERR_bad) {

  LINE(77);
        {
        CLUREF T_2_1;
        CellAlloc(2, nil, T_2_1);
        abs.num = T_2_1.num;
        }
    }
    else { /* not handled */
        goto ex_0;
    }
  end_1:;

  LINE(78);
    { /* return */
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = stmtOPcreate(abs, l, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    err = p_except(e, T_1_1, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    ret_1->num = T_1_2.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_stmt ****/



/**** BEGIN PROCEDURE p_if ****/

extern errcode sequenceOPnew();
extern errcode p_expr1();
extern errcode p_envOPassume();
extern errcode sequenceOPaddh();
extern errcode oneofOPmake_1();
extern errcode oneofOPmake_12();
static CLUREF STR_THEN_040in_040IF_040statement;
static CLUREF STR_END_040of_040IF_040statement;
static int p_if_own_init = 0;

errcode
p_if(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF arms;
    CLUREF else_;
    CLUREF line;
    CLUREF x;
    CLUREF bod;
    if (p_if_own_init == 0) {
        stringOPcons("THEN in IF statement", CLU_1, CLUREF_make_num(20), &STR_THEN_040in_040IF_040statement);
        stringOPcons("END of IF statement", CLU_1, CLUREF_make_num(19), &STR_END_040of_040IF_040statement);
        p_if_own_init = 1;
    }
    enter_proc(85);

  LINE(86);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    arms.num = T_1_1.num;
    }

  LINE(87);
    {
    CLUREF T_1_1;
    CellAlloc(2, nil, T_1_1);
    else_.num = T_1_1.num;
    }

  LINE(88);
    for (;;) { /* while */
        if (true != true)
            break;

  LINE(89);
        {
        CLUREF T_2_1;
        err = p_envOPget_peekline(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        line.num = T_2_1.num;
        }

  LINE(90);
        {
        CLUREF T_2_1;
        err = p_expr1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        x.num = T_2_1.num;
        }

  LINE(91);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 36: /* then_ */ {

  LINE(92);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }
            break;
        }
        default: {

  LINE(93);
            {
            err = p_envOPassume(e, STR_THEN_040in_040IF_040statement);
            if (err != ERR_ok)
                goto ex_0;
            }
            break;
        }
        }
        } /* end qtagcase */

  LINE(95);
        {
        CLUREF T_2_1;
        err = p_body(e, CLU_0, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        bod.num = T_2_1.num;
        }

  LINE(96);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        RecordAlloc(3, T_2_1);
        T_2_1.vec->data[2] = x.num;
        T_2_1.vec->data[0] = bod.num;
        T_2_1.vec->data[1] = line.num;
        err = sequenceOPaddh(arms, T_2_1, &T_2_2);
        if (err != ERR_ok)
            goto ex_0;
        arms.num = T_2_2.num;
        }

  LINE(99);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 8: /* elseif_ */ {

  LINE(100);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(101);
            continue;
            break;
        }
        case 7: /* else_ */ {

  LINE(102);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(103);
            {
            CLUREF T_3_1;
            CLUREF T_3_2;
            err = p_body(e, CLU_3, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            CellAlloc(1, T_3_1.num, T_3_2);
            else_.num = T_3_2.num;
            }
            break;
        }
        }
        } /* end qtagcase */

  LINE(106);
        {
        err = p_end(e, STR_END_040of_040IF_040statement);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(108);
        { /* return */
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        RecordAlloc(2, T_2_1);
        T_2_1.vec->data[0] = arms.num;
        T_2_1.vec->data[1] = else_.num;
        CellAlloc(12, T_2_1.num, T_2_2);
        ret_1->num = T_2_2.num;
        }
        signal (ERR_ok);
        }
    }
    end_while_1:
    __CLU_END_LABEL;

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_if ****/



/**** BEGIN PROCEDURE p_for ****/

extern errcode p_idns_or_decls();
extern errcode p_make_invoke();
extern errcode p_do();
extern errcode oneofOPmake_10();
static CLUREF STR_use_040of_040_072_075_040instead_040of_040IN;
static CLUREF STR_IN_040in_040FOR_040statement;
static CLUREF STR_expression_040not_040an_040invocation;
static int p_for_own_init = 0;

errcode
p_for(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF vars;
    CLUREF call;
    CLUREF bod;
    if (p_for_own_init == 0) {
        stringOPcons("use of := instead of IN", CLU_1, CLUREF_make_num(23), &STR_use_040of_040_072_075_040instead_040of_040IN);
        stringOPcons("IN in FOR statement", CLU_1, CLUREF_make_num(19), &STR_IN_040in_040FOR_040statement);
        stringOPcons("expression not an invocation", CLU_1, CLUREF_make_num(28), &STR_expression_040not_040an_040invocation);
        p_for_own_init = 1;
    }
    enter_proc(115);

  LINE(117);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 17: /* in_ */ {

  LINE(118);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        err = sequenceOPnew(&T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        CellAlloc(2, T_2_1.num, T_2_2);
        vars.num = T_2_2.num;
        }
        break;
    }
    default: {

  LINE(119);
        {
        CLUREF T_2_1;
        err = p_idns_or_decls(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        vars.num = T_2_1.num;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(121);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 17: /* in_ */ {

  LINE(122);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    case 1: /* assn */ {

  LINE(123);
        {
        err = p_envOPerr(e, STR_use_040of_040_072_075_040instead_040of_040IN, CLU_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(124);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    default: {

  LINE(125);
        {
        err = p_envOPassume(e, STR_IN_040in_040FOR_040statement);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(127);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = p_expr1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    err = p_make_invoke(e, T_1_1, STR_expression_040not_040an_040invocation, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    call.num = T_1_2.num;
    }

  LINE(129);
    {
    CLUREF T_1_1;
    err = p_do(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    bod.num = T_1_1.num;
    }

  LINE(130);
    { /* return */
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
    RecordAlloc(5, T_1_1);
    T_1_1.vec->data[3] = vars.num;
    err = sequenceOPnew(&T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.vec->data[0] = T_1_2.num;
    T_1_1.vec->data[2] = call.num;
    T_1_1.vec->data[1] = bod.num;
    T_1_1.vec->data[4] = false;
    CellAlloc(10, T_1_1.num, T_1_3);
    ret_1->num = T_1_3.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_for ****/



/**** BEGIN PROCEDURE p_do ****/

static CLUREF STR_DO_040of_040DO_056_056_056END_040clause;
static CLUREF STR_END_040of_040DO_056_056_056END_040clause;
static int p_do_own_init = 0;

errcode
p_do(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF bod;
    if (p_do_own_init == 0) {
        stringOPcons("DO of DO...END clause", CLU_1, CLUREF_make_num(21), &STR_DO_040of_040DO_056_056_056END_040clause);
        stringOPcons("END of DO...END clause", CLU_1, CLUREF_make_num(22), &STR_END_040of_040DO_056_056_056END_040clause);
        p_do_own_init = 1;
    }
    enter_proc(139);

  LINE(140);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 5: /* do_ */ {

  LINE(141);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    default: {

  LINE(142);
        {
        err = p_envOPassume(e, STR_DO_040of_040DO_056_056_056END_040clause);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(144);
    {
    CLUREF T_1_1;
    err = p_body(e, CLU_3, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    bod.num = T_1_1.num;
    }

  LINE(145);
    {
    err = p_end(e, STR_END_040of_040DO_056_056_056END_040clause);
    if (err != ERR_ok)
        goto ex_0;
    }

  LINE(146);
    { /* return */
    {
    ret_1->num = bod.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_do ****/



/**** BEGIN PROCEDURE p_idns_or_decls ****/

extern errcode p_clist();
extern errcode p_idn();
extern errcode p_type1();
extern errcode p_decl();
extern errcode sequenceOPaddl();
extern const struct REQS * const p_clist_of_t_reqs;
extern struct OPS *idn_ops;
extern const OWN_req p_clist_ownreqs;
struct OPS *p_clist_of_idn_ops __CLU_COMMON;
extern const struct REQS * const sequence_of_t_reqs;
extern const OWN_req sequence_ownreqs;
extern struct OPS *sequence_ops;
struct OPS *sequence_of_idn_ops __CLU_COMMON;
extern struct OPS *sequence_of_idn_ops;
extern struct OPS *typespec_ops;
struct OPS *record_idns_type__ops __CLU_COMMON;
extern struct OPS *record_idns_type__ops;
struct OPS *p_clist_of_record_idns_type__ops __CLU_COMMON;
static CLUREF STR_idn;
static CLUREF STR_decl;
static int p_idns_or_decls_own_init = 0;

errcode
p_idns_or_decls(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF idns;
    CLUREF t;
    CLUREF decls;
    if (p_idns_or_decls_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)idn_ops, p_clist_of_t_reqs);
        find_prociter_instance(p_clist, 1, &p_clist_ownreqs, &(p_clist_of_idn_ops));
        add_parm_info_type(0, (const struct OPS *)idn_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_idn_ops));
        add_selector_info("idns", 0, (struct OPS *)sequence_of_idn_ops);
        add_selector_info("type_", 1, (struct OPS *)typespec_ops);
        find_selector_ops("record", 2, &(record_idns_type__ops));
        add_parm_info_type(0, (const struct OPS *)record_idns_type__ops, p_clist_of_t_reqs);
        find_prociter_instance(p_clist, 1, &p_clist_ownreqs, &(p_clist_of_record_idns_type__ops));
        stringOPcons("idn", CLU_1, CLUREF_make_num(3), &STR_idn);
        stringOPcons("decl", CLU_1, CLUREF_make_num(4), &STR_decl);
        p_idns_or_decls_own_init = 1;
    }
    enter_proc(151);

  LINE(152);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = proctypeOPnew(CLU_0, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.proc->type_owns = NULL;
    T_1_1.proc->op_owns = NULL;
    T_1_1.proc->proc = p_idn;
    generic_CLU_proc.type_owns = NULL;
    generic_CLU_proc.op_owns = p_clist_of_idn_ops->op_owns;
    generic_CLU_proc.proc = p_clist;
    CUR_PROC_VAR.proc = &generic_CLU_proc;
    err = p_clist(e, T_1_1, STR_idn, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    idns.num = T_1_2.num;
    }

  LINE(153);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 2: /* colon */ {

  LINE(155);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(156);
        {
        CLUREF T_2_1;
        err = p_type1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        t.num = T_2_1.num;
        }

  LINE(158);
        { /* qtagcase */
        CLUREF T_2_1;
        err = p_envOPget_peek1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        switch (T_2_1.cell->tag) {
        case 3: /* comma */ {

  LINE(159);
            {
            CLUREF T_3_1;
            err = p_envOPnext_token(e, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            }

  LINE(160);
            {
            CLUREF T_3_1;
            CLUREF T_3_2;
            err = proctypeOPnew(CLU_0, &T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            T_3_1.proc->type_owns = NULL;
            T_3_1.proc->op_owns = NULL;
            T_3_1.proc->proc = p_decl;
            generic_CLU_proc.type_owns = NULL;
            generic_CLU_proc.op_owns = p_clist_of_record_idns_type__ops->op_owns;
            generic_CLU_proc.proc = p_clist;
            CUR_PROC_VAR.proc = &generic_CLU_proc;
            err = p_clist(e, T_3_1, STR_decl, &T_3_2);
            if (err != ERR_ok)
                goto ex_0;
            decls.num = T_3_2.num;
            }
            break;
        }
        default: {

  LINE(161);
            {
            CLUREF T_3_1;
            err = sequenceOPnew(&T_3_1);
            if (err != ERR_ok)
                goto ex_0;
            decls.num = T_3_1.num;
            }
            break;
        }
        }
        } /* end qtagcase */

  LINE(163);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        RecordAlloc(2, T_2_1);
        T_2_1.vec->data[0] = idns.num;
        T_2_1.vec->data[1] = t.num;
        err = sequenceOPaddl(decls, T_2_1, &T_2_2);
        if (err != ERR_ok)
            goto ex_0;
        decls.num = T_2_2.num;
        }

  LINE(165);
        { /* return */
        {
        CLUREF T_2_1;
        CellAlloc(1, decls.num, T_2_1);
        ret_1->num = T_2_1.num;
        }
        signal (ERR_ok);
        }
        break;
    }
    default: {

  LINE(167);
        { /* return */
        {
        CLUREF T_2_1;
        CellAlloc(2, idns.num, T_2_1);
        ret_1->num = T_2_1.num;
        }
        signal (ERR_ok);
        }
        break;
    }
    }
    } /* end qtagcase */

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_idns_or_decls ****/



/**** BEGIN PROCEDURE p_idn ****/


errcode
p_idn(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF k;
    enter_proc(173);

  LINE(174);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 16: /* idn */ {
        CLUREF T_1_2;
        T_1_2.num = T_1_1.cell->value;
        k.num = T_1_2.num;

  LINE(176);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(177);
        { /* return */
        {
        ret_1->num = k.num;
        }
        signal (ERR_ok);
        }
        break;
    }
    default: {

  LINE(179);
        { /* signal */
            signal(ERR_none);
        }
        break;
    }
    }
    } /* end qtagcase */

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_idn ****/



/**** BEGIN ITERATOR p_idn1 ****/


errcode
p_idn1(CLUREF e, errcode (*proc)(), void *user_locals, errcode *iecode)
{
    errcode ecode __CLU_UNUSED;
    errcode err;
    enter_proc(183);

  LINE(184);
    { /* yield */
    CLUREF T_2_1;
    err = p_idn(e, &T_2_1);
    if (err != ERR_ok)
        goto ex_1;
    err = (*proc)(T_2_1, user_locals, iecode);
    if (err != ERR_ok) {
        signal(err);
    }
    }
    goto end_1;
  ex_1: /* except */
    __CLU_EX_HANDLER;
    if (err == ERR_none) {
    }
    else { /* not handled */
        goto ex_0;
    }
  end_1:;

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    signal(ERR_ok);
}

/**** END ITERATOR p_idn1 ****/



/**** BEGIN PROCEDURE p_decl ****/

extern errcode p_envOPget_unknown_type();
struct OPS *p_clist_of_idn_ops __CLU_COMMON;
static CLUREF STR__072_040type_040in_040declaration;
static int p_decl_own_init = 0;

errcode
p_decl(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF idns;
    CLUREF t;
    if (p_decl_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)idn_ops, p_clist_of_t_reqs);
        find_prociter_instance(p_clist, 1, &p_clist_ownreqs, &(p_clist_of_idn_ops));
        stringOPcons("idn", CLU_1, CLUREF_make_num(3), &STR_idn);
        stringOPcons(": type in declaration", CLU_1, CLUREF_make_num(21), &STR__072_040type_040in_040declaration);
        p_decl_own_init = 1;
    }
    enter_proc(192);

  LINE(193);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = proctypeOPnew(CLU_0, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.proc->type_owns = NULL;
    T_1_1.proc->op_owns = NULL;
    T_1_1.proc->proc = p_idn;
    generic_CLU_proc.type_owns = NULL;
    generic_CLU_proc.op_owns = p_clist_of_idn_ops->op_owns;
    generic_CLU_proc.proc = p_clist;
    CUR_PROC_VAR.proc = &generic_CLU_proc;
    err = p_clist(e, T_1_1, STR_idn, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    idns.num = T_1_2.num;
    }

  LINE(195);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 2: /* colon */ {

  LINE(196);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(197);
        {
        CLUREF T_2_1;
        err = p_type1(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        t.num = T_2_1.num;
        }
        break;
    }
    default: {

  LINE(198);
        {
        err = p_envOPassume(e, STR__072_040type_040in_040declaration);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(199);
        {
        CLUREF T_2_1;
        err = p_envOPget_unknown_type(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        t.num = T_2_1.num;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(201);
    { /* return */
    {
    CLUREF T_1_1;
    RecordAlloc(2, T_1_1);
    T_1_1.vec->data[0] = idns.num;
    T_1_1.vec->data[1] = t.num;
    ret_1->num = T_1_1.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_decl ****/



/**** BEGIN PROCEDURE p_tagcase ****/

extern errcode p_list();
extern errcode p_tagarm();
extern errcode oneofOPmake_18();
extern errcode recordOPset_3();
extern errcode p_cbody();
extern struct OPS *equate_ops;
struct OPS *sequence_of_equate_ops __CLU_COMMON;
extern struct OPS *stmt_ops;
struct OPS *sequence_of_stmt_ops __CLU_COMMON;
extern struct OPS *sequence_of_equate_ops;
extern struct OPS *sequence_of_stmt_ops;
struct OPS *record_equates_stmts_ops __CLU_COMMON;
extern struct OPS *string_ops;
struct OPS *sequence_of_string_ops __CLU_COMMON;
struct OPS *sequence_of_idn_ops __CLU_COMMON;
struct OPS *record_idns_type__ops __CLU_COMMON;
extern struct OPS *null_ops;
struct OPS *oneof_decl_none_ops __CLU_COMMON;
extern struct OPS *record_equates_stmts_ops;
extern struct OPS *int_ops;
extern struct OPS *sequence_of_string_ops;
extern struct OPS *oneof_decl_none_ops;
struct OPS *record_body_line_tags_var_ops __CLU_COMMON;
extern const struct REQS * const p_list_of_t_reqs;
extern struct OPS *record_body_line_tags_var_ops;
extern const OWN_req p_list_ownreqs;
struct OPS *p_list_of_record_body_line_tags_var_ops __CLU_COMMON;
static CLUREF STR_use_040of_040IN_040after_040tagcase_040expression;
static CLUREF STR_TAG_040arm;
static CLUREF STR__072_040in_040OTHERS_072_040body;
static CLUREF STR_END_040of_040TAGCASE_040statement;
static int p_tagcase_own_init = 0;

errcode
p_tagcase(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF obj;
    CLUREF arms;
    CLUREF tst;
    CLUREF abs;
    if (p_tagcase_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)equate_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_equate_ops));
        add_parm_info_type(0, (const struct OPS *)stmt_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_stmt_ops));
        add_selector_info("equates", 0, (struct OPS *)sequence_of_equate_ops);
        add_selector_info("stmts", 1, (struct OPS *)sequence_of_stmt_ops);
        find_selector_ops("record", 2, &(record_equates_stmts_ops));
        add_parm_info_type(0, (const struct OPS *)string_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_string_ops));
        add_parm_info_type(0, (const struct OPS *)idn_ops, sequence_of_t_reqs);
        find_type_instance(sequence_ops, 1, &sequence_ownreqs, &(sequence_of_idn_ops));
        add_selector_info("idns", 0, (struct OPS *)sequence_of_idn_ops);
        add_selector_info("type_", 1, (struct OPS *)typespec_ops);
        find_selector_ops("record", 2, &(record_idns_type__ops));
        add_selector_info("decl", 0, (struct OPS *)record_idns_type__ops);
        add_selector_info("none", 1, (struct OPS *)null_ops);
        find_selector_ops("oneof", 2, &(oneof_decl_none_ops));
        add_selector_info("body", 0, (struct OPS *)record_equates_stmts_ops);
        add_selector_info("line", 1, (struct OPS *)int_ops);
        add_selector_info("tags", 2, (struct OPS *)sequence_of_string_ops);
        add_selector_info("var", 3, (struct OPS *)oneof_decl_none_ops);
        find_selector_ops("record", 4, &(record_body_line_tags_var_ops));
        add_parm_info_type(0, (const struct OPS *)record_body_line_tags_var_ops, p_list_of_t_reqs);
        find_prociter_instance(p_list, 1, &p_list_ownreqs, &(p_list_of_record_body_line_tags_var_ops));
        stringOPcons("use of IN after tagcase expression", CLU_1, CLUREF_make_num(34), &STR_use_040of_040IN_040after_040tagcase_040expression);
        stringOPcons("TAG arm", CLU_1, CLUREF_make_num(7), &STR_TAG_040arm);
        stringOPcons(": in OTHERS: body", CLU_1, CLUREF_make_num(17), &STR__072_040in_040OTHERS_072_040body);
        stringOPcons("END of TAGCASE statement", CLU_1, CLUREF_make_num(24), &STR_END_040of_040TAGCASE_040statement);
        p_tagcase_own_init = 1;
    }
    enter_proc(209);

  LINE(210);
    {
    CLUREF T_1_1;
    err = p_expr1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    obj.num = T_1_1.num;
    }

  LINE(211);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 17: /* in_ */ {

  LINE(212);
        {
        err = p_envOPerr(e, STR_use_040of_040IN_040after_040tagcase_040expression, CLU_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(213);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(216);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = proctypeOPnew(CLU_0, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.proc->type_owns = NULL;
    T_1_1.proc->op_owns = NULL;
    T_1_1.proc->proc = p_tagarm;
    generic_CLU_proc.type_owns = NULL;
    generic_CLU_proc.op_owns = p_list_of_record_body_line_tags_var_ops->op_owns;
    generic_CLU_proc.proc = p_list;
    CUR_PROC_VAR.proc = &generic_CLU_proc;
    err = p_list(e, T_1_1, CLU_false, STR_TAG_040arm, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    arms.num = T_1_2.num;
    }

  LINE(217);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    RecordAlloc(3, T_1_1);
    T_1_1.vec->data[1] = obj.num;
    T_1_1.vec->data[0] = arms.num;
    CellAlloc(2, nil, T_1_2);
    T_1_1.vec->data[2] = T_1_2.num;
    tst.num = T_1_1.num;
    }

  LINE(220);
    {
    CLUREF T_1_1;
    CellAlloc(18, tst.num, T_1_1);
    abs.num = T_1_1.num;
    }

  LINE(221);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 24: /* others_ */ {

  LINE(223);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }

  LINE(224);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        err = p_cbody(e, CLU_3, STR__072_040in_040OTHERS_072_040body, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        CellAlloc(1, T_2_1.num, T_2_2);
        tst.vec->data[2] = T_2_2.num;
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(228);
    {
    err = p_end(e, STR_END_040of_040TAGCASE_040statement);
    if (err != ERR_ok)
        goto ex_0;
    }

  LINE(229);
    { /* return */
    {
    ret_1->num = abs.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_tagcase ****/



/**** BEGIN PROCEDURE p_tagarm ****/

extern errcode p_envOPget_line();
extern errcode p_name();
extern errcode p_mdecl();
struct OPS *p_clist_of_string_ops __CLU_COMMON;
static CLUREF STR_name;
static CLUREF STR__072_040in_040TAG_056_056_056_072_040body;
static int p_tagarm_own_init = 0;

errcode
p_tagarm(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF line;
    CLUREF tags;
    CLUREF var;
    CLUREF bod;
    if (p_tagarm_own_init == 0) {
        add_parm_info_type(0, (const struct OPS *)string_ops, p_clist_of_t_reqs);
        find_prociter_instance(p_clist, 1, &p_clist_ownreqs, &(p_clist_of_string_ops));
        stringOPcons("name", CLU_1, CLUREF_make_num(4), &STR_name);
        stringOPcons(": in TAG...: body", CLU_1, CLUREF_make_num(17), &STR__072_040in_040TAG_056_056_056_072_040body);
        p_tagarm_own_init = 1;
    }
    enter_proc(234);

  LINE(235);
    { /* qtagcase */
    CLUREF T_1_1;
    err = p_envOPget_peek1(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    switch (T_1_1.cell->tag) {
    case 35: /* tag_ */ {

  LINE(236);
        {
        CLUREF T_2_1;
        err = p_envOPnext_token(e, &T_2_1);
        if (err != ERR_ok)
            goto ex_0;
        }
        break;
    }
    default: {

  LINE(237);
        { /* signal */
            signal(ERR_none);
        }
        break;
    }
    }
    } /* end qtagcase */

  LINE(239);
    {
    CLUREF T_1_1;
    err = p_envOPget_line(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    line.num = T_1_1.num;
    }

  LINE(240);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    err = proctypeOPnew(CLU_0, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    T_1_1.proc->type_owns = NULL;
    T_1_1.proc->op_owns = NULL;
    T_1_1.proc->proc = p_name;
    generic_CLU_proc.type_owns = NULL;
    generic_CLU_proc.op_owns = p_clist_of_string_ops->op_owns;
    generic_CLU_proc.proc = p_clist;
    CUR_PROC_VAR.proc = &generic_CLU_proc;
    err = p_clist(e, T_1_1, STR_name, &T_1_2);
    if (err != ERR_ok)
        goto ex_0;
    tags.num = T_1_2.num;
    }

  LINE(241);
    {
    CLUREF T_1_1;
    err = p_mdecl(e, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    var.num = T_1_1.num;
    }

  LINE(242);
    {
    CLUREF T_1_1;
    err = p_cbody(e, CLU_1, STR__072_040in_040TAG_056_056_056_072_040body, &T_1_1);
    if (err != ERR_ok)
        goto ex_0;
    bod.num = T_1_1.num;
    }

  LINE(243);
    { /* return */
    {
    CLUREF T_1_1;
    RecordAlloc(4, T_1_1);
    T_1_1.vec->data[2] = tags.num;
    T_1_1.vec->data[3] = var.num;
    T_1_1.vec->data[0] = bod.num;
    T_1_1.vec->data[1] = line.num;
    ret_1->num = T_1_1.num;
    }
    signal (ERR_ok);
    }

    goto end_0;
  ex_0:
    __CLU_EX_HANDLER;
    if (err != ERR_failure)
        elist[0] = _pclu_erstr(err);
    signal(ERR_failure);
  end_0:
    elist[0] = no_return_values_STRING;
    signal(ERR_failure);
}

/**** END PROCEDURE p_tagarm ****/

