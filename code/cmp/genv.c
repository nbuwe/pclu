
/* This file was automatically generated by pclu.*/

#include "pclu_err.h"
#include "pclu_sys.h"


/**** BEGIN CLUSTER g_env ****/

extern errcode sequenceOPnew();
extern errcode streamOPopen();
extern errcode arrayOPpredict();
extern errcode oneofOPmake_1();
extern errcode arrayOPtrim();
extern errcode recordOPget_4();
extern errcode recordOPset_9();
extern errcode recordOPset_8();
extern errcode streamOPclose();
extern errcode recordOPget_10();
extern errcode recordOPset_10();
extern errcode streamOPputl();
extern errcode recordOPget_7();
extern errcode stringOPconcat();
extern errcode recordOPset_7();
extern errcode intOPgt();
extern errcode arrayOPsize();
extern errcode g_envOPoutput_line();
extern errcode recordOPget_12();
extern errcode recordOPset_12();
extern errcode recordOPset_11();
extern errcode recordOPget_11();
extern errcode recordOPset_13();
extern errcode recordOPget_13();
extern errcode recordOPget_8();
extern errcode boolOPnot();
extern errcode recordOPget_6();
extern errcode g_envOPnewline();
extern errcode arrayOPaddh();
extern errcode intOPequal();
extern errcode stringOPsize();
extern errcode recordOPget_5();
extern errcode recordOPget_9();
extern errcode recordOPset_5();
extern errcode stringOPsubstr();
extern errcode recordOPset_6();
extern errcode intOPadd();
extern errcode arrayOPempty();
extern errcode intOPsub();
extern errcode streamOPputs();
extern errcode intOPunparse();
extern errcode streamOPputc();
extern errcode recordOPset_2();
extern errcode oneofOPmake_2();
extern errcode recordOPset_3();
extern errcode recordOPget_3();
extern errcode recordOPget_2();
extern errcode ibOPbodies();
extern errcode arrayOPelements();
extern errcode recordOPget_1();
extern errcode sequenceOPelements();
extern errcode stringOPequal();
extern errcode idnOPget_str();
extern errcode sequenceOPaddh();
extern errcode intOPlt();
static CLUREF STR_write;
static CLUREF STR_;
static CLUREF STR__012_057_052_040This_040file_040was_040automatically_040generated_040by_040pclu_056_052_057_012;
static CLUREF STR__043include_040_042pclu_137err_056h_042;
static CLUREF STR__043include_040_042pclu_137sys_056h_042;
static CLUREF STR_can_047t_040write_040to_040binary_040file_072_040;
static CLUREF STR_codegen_072_040_043SIGNAL_040directive_040ignored;
static CLUREF STR__040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040;
static CLUREF STR__040_040FB_137LINE_050;
static CLUREF STR__051_073;
static CLUREF STR__040_040LINE_050;
static int g_env_own_init = 0;
const OWN_req g_env_ownreqs = { 0, 0 };
CLUREF g_envOPcurrent_env;
CLUREF g_envOPlast_lineno;
CLUREF g_envOPlineno;
CLUREF g_envOPtypedef_list;
CLUREF g_envOPglobal_list;
CLUREF g_envOPstr_list;
CLUREF g_envOPextern_list;
CLUREF g_envOPvlist;
CLUREF g_envOPsiglist;
CLUREF g_envOPfind_owns_list;

errcode
g_env_own_init_proc(void)
{
    errcode err;
    enter_own_init_proc();
    if (g_env_own_init == 0) {
        stringOPcons("write", CLU_1, CLUREF_make_num(5), &STR_write);
        stringOPcons("", CLU_1, CLUREF_make_num(0), &STR_);
        stringOPcons("\n/* This file was automatically generated by pclu.*/\n", CLU_1, CLUREF_make_num(53), &STR__012_057_052_040This_040file_040was_040automatically_040generated_040by_040pclu_056_052_057_012);
        stringOPcons("#include \"pclu_err.h\"", CLU_1, CLUREF_make_num(21), &STR__043include_040_042pclu_137err_056h_042);
        stringOPcons("#include \"pclu_sys.h\"", CLU_1, CLUREF_make_num(21), &STR__043include_040_042pclu_137sys_056h_042);
        stringOPcons("can\'t write to binary file: ", CLU_1, CLUREF_make_num(28), &STR_can_047t_040write_040to_040binary_040file_072_040);
        stringOPcons("codegen: #SIGNAL directive ignored", CLU_1, CLUREF_make_num(34), &STR_codegen_072_040_043SIGNAL_040directive_040ignored);
        stringOPcons("                                                                             ", CLU_1, CLUREF_make_num(77), &STR__040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040);
        stringOPcons("  FB_LINE(", CLU_1, CLUREF_make_num(10), &STR__040_040FB_137LINE_050);
        stringOPcons(");", CLU_1, CLUREF_make_num(2), &STR__051_073);
        stringOPcons("  LINE(", CLU_1, CLUREF_make_num(7), &STR__040_040LINE_050);
        g_env_own_init = 1;
        {
            {g_envOPlast_lineno.num = 0;
            }
            }
        {
            {g_envOPlineno.num = 0;
            }
            }
        {
            {CLUREF T_0_1;
            err = sequenceOPnew(&T_0_1);
            if (err != ERR_ok) goto ex_0;
            g_envOPtypedef_list.num = T_0_1.num;
            }
            }
        {
            {CLUREF T_0_2;
            err = sequenceOPnew(&T_0_2);
            if (err != ERR_ok) goto ex_0;
            g_envOPglobal_list.num = T_0_2.num;
            }
            }
        {
            {CLUREF T_0_3;
            err = sequenceOPnew(&T_0_3);
            if (err != ERR_ok) goto ex_0;
            g_envOPstr_list.num = T_0_3.num;
            }
            }
        {
            {CLUREF T_0_4;
            err = sequenceOPnew(&T_0_4);
            if (err != ERR_ok) goto ex_0;
            g_envOPextern_list.num = T_0_4.num;
            }
            }
        {
            {CLUREF T_0_5;
            err = sequenceOPnew(&T_0_5);
            if (err != ERR_ok) goto ex_0;
            g_envOPvlist.num = T_0_5.num;
            }
            }
        {
            {CLUREF T_0_6;
            err = sequenceOPnew(&T_0_6);
            if (err != ERR_ok) goto ex_0;
            g_envOPsiglist.num = T_0_6.num;
            }
            }
        {
            {CLUREF T_0_7;
            err = sequenceOPnew(&T_0_7);
            if (err != ERR_ok) goto ex_0;
            g_envOPfind_owns_list.num = T_0_7.num;
            }
            }
        signal(ERR_ok);
      ex_0:
        pclu_unhandled(err);
        signal(ERR_failure);
    }
    signal(ERR_ok);
}


/**** BEGIN PROCEDURE create ****/

static int g_envOPcreate_own_init = 0;

errcode
g_envOPcreate(CLUREF comp, CLUREF errst, CLUREF binfn, CLUREF *ret_1)
{
    errcode err;
    if (g_envOPcreate_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPcreate_own_init = 1;
    }
    enter_proc(45);

  LINE(46);
    {
    g_envOPlineno.num = 0;
    }

  LINE(47);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
    CLUREF T_1_4;
    RecordAlloc(13, T_1_1);
    err = streamOPopen(binfn, STR_write, &T_1_2);
    if (err != ERR_ok) goto ex_0;
    T_1_1.vec->data[9] = T_1_2.num;
    T_1_1.vec->data[6] = errst.num;
    err = arrayOPpredict(CLU_1, CLU_20, &T_1_3);
    if (err != ERR_ok) goto ex_0;
    T_1_1.vec->data[3] = T_1_3.num;
    T_1_1.vec->data[4] = STR_.num;
    T_1_1.vec->data[8] = 0;
    T_1_1.vec->data[7] = 0;
    T_1_1.vec->data[5] = false;
    CellAlloc(1, nil, T_1_4);
    T_1_1.vec->data[1] = T_1_4.num;
    T_1_1.vec->data[2] = false;
    T_1_1.vec->data[11] = binfn.num;
    T_1_1.vec->data[0] = comp.num;
    T_1_1.vec->data[12] = false;
    T_1_1.vec->data[10] = false;
    g_envOPcurrent_env.num = T_1_1.num;
    }

  LINE(61);
    {
    {
    ret_1->num = g_envOPcurrent_env.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE create ****/


/**** BEGIN PROCEDURE reset ****/

static int g_envOPreset_own_init = 0;

errcode
g_envOPreset(CLUREF e, CLUREF comp, CLUREF errst, CLUREF binfn)
{
    errcode err;
    CLUREF s;
    if (g_envOPreset_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPreset_own_init = 1;
    }
    enter_proc(64);

  LINE(65);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[3];
    err = arrayOPtrim(T_1_1, CLU_1, CLU_0);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(66);
    {
    g_envOPlineno.num = 0;
    }

  LINE(67);
    {
    e.vec->data[8] = 0;
    }

  LINE(68);
    {
    e.vec->data[7] = 0;
    }

  LINE(69);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPtypedef_list.num = T_1_1.num;
    }

  LINE(70);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPglobal_list.num = T_1_1.num;
    }

  LINE(71);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPstr_list.num = T_1_1.num;
    }

  LINE(72);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPextern_list.num = T_1_1.num;
    }

  LINE(73);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPvlist.num = T_1_1.num;
    }

  LINE(74);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPsiglist.num = T_1_1.num;
    }

  LINE(75);
    {
    CLUREF T_1_1;
    err = sequenceOPnew(&T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPfind_owns_list.num = T_1_1.num;
    }

  LINE(76);
    {
    CLUREF T_2_1;
    T_2_1.num = e.vec->data[9];
    err = streamOPclose(T_2_1);
    if (err != ERR_ok) goto ex_1;
    }
        goto end_1;
        ex_1:
            if ((err == ERR_not_possible)) {
            }
            else {
                goto ex_0;
            }
        end_1:;

  LINE(78);
    {

  LINE(79);
        {
        CLUREF T_3_1;
        err = streamOPopen(binfn, STR_write, &T_3_1);
        if (err != ERR_ok) goto ex_2;
        e.vec->data[9] = T_3_1.num;
        }

  LINE(80);
        {
        CLUREF T_3_1;
        T_3_1.num = e.vec->data[9];
        err = streamOPputl(T_3_1, STR__012_057_052_040This_040file_040was_040automatically_040generated_040by_040pclu_056_052_057_012);
        if (err != ERR_ok) goto ex_2;
        }

  LINE(81);
        {
        CLUREF T_3_1;
        T_3_1.num = e.vec->data[9];
        err = streamOPputl(T_3_1, STR__043include_040_042pclu_137err_056h_042);
        if (err != ERR_ok) goto ex_2;
        }

  LINE(82);
        {
        CLUREF T_3_1;
        T_3_1.num = e.vec->data[9];
        err = streamOPputl(T_3_1, STR__043include_040_042pclu_137sys_056h_042);
        if (err != ERR_ok) goto ex_2;
        }
        }
        goto end_2;
        ex_2:
            if ((err == ERR_not_possible)) {
            CLUREF s;
            s.num = elist[0].num;

  LINE(88);
                {
                CLUREF T_2_1;
                CLUREF T_2_2;
                T_2_1.num = e.vec->data[6];
                err = stringOPconcat(STR_can_047t_040write_040to_040binary_040file_072_040, s, &T_2_2);
                if (err != ERR_ok) goto ex_0;
                err = streamOPputl(T_2_1, T_2_2);
                if (err != ERR_ok) goto ex_0;
                }

  LINE(89);
                {
                e.vec->data[7] = 2;
                }
            }
            else {
                goto ex_0;
            }
        end_2:;

  LINE(91);
    {
    e.vec->data[6] = errst.num;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE reset ****/


/**** BEGIN PROCEDURE end_all ****/


errcode
g_envOPend_all(CLUREF e)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(94);

  LINE(95);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
    T_1_1.num = e.vec->data[3];
    T_1_2.num = T_1_1.array->ext_size;
    T_1_3.num = (T_1_2.num > 0)? true : false;
    if (T_1_3.num == true) {

  LINE(96);
        {
        err = g_envOPoutput_line(e);
        if (err != ERR_ok) goto ex_0;
        }
        }
        }/* end if */

  LINE(98);
    {
    CLUREF T_2_1;
    T_2_1.num = e.vec->data[9];
    err = streamOPclose(T_2_1);
    if (err != ERR_ok) goto ex_1;
    }
        goto end_1;
        ex_1:
            if ((err == ERR_not_possible)) {
            }
            else {
                goto ex_0;
            }
        end_1:;
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE end_all ****/


/**** BEGIN PROCEDURE get_file_name ****/


errcode
g_envOPget_file_name(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(102);

  LINE(103);
    {
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[11];
    ret_1->num = T_1_1.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_file_name ****/


/**** BEGIN PROCEDURE set_file_name ****/


errcode
g_envOPset_file_name(CLUREF e, CLUREF fn)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(106);

  LINE(107);
    {
    e.vec->data[11] = fn.num;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE set_file_name ****/


/**** BEGIN PROCEDURE force_signal ****/

static int g_envOPforce_signal_own_init = 0;

errcode
g_envOPforce_signal(CLUREF e)
{
    errcode err;
    if (g_envOPforce_signal_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPforce_signal_own_init = 1;
    }
    enter_proc(110);

  LINE(111);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[6];
    err = streamOPputl(T_1_1, STR_codegen_072_040_043SIGNAL_040directive_040ignored);
    if (err != ERR_ok) goto ex_0;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE force_signal ****/


/**** BEGIN PROCEDURE reset_err ****/


errcode
g_envOPreset_err(CLUREF e)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(114);
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE reset_err ****/


/**** BEGIN PROCEDURE set_space ****/


errcode
g_envOPset_space(CLUREF e, CLUREF b)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(117);

  LINE(118);
    {
    e.vec->data[10] = b.num;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE set_space ****/


/**** BEGIN PROCEDURE get_space ****/


errcode
g_envOPget_space(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(121);

  LINE(122);
    {
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[10];
    ret_1->num = T_1_1.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_space ****/


/**** BEGIN PROCEDURE set_time ****/


errcode
g_envOPset_time(CLUREF e, CLUREF b)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(125);

  LINE(126);
    {
    e.vec->data[12] = b.num;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE set_time ****/


/**** BEGIN PROCEDURE get_time ****/


errcode
g_envOPget_time(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(129);

  LINE(130);
    {
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[12];
    ret_1->num = T_1_1.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_time ****/


/**** BEGIN PROCEDURE get_err ****/


errcode
g_envOPget_err(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(133);

  LINE(134);
    {
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[7];
    ret_1->num = T_1_1.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_err ****/


/**** BEGIN PROCEDURE set_lineno ****/


errcode
g_envOPset_lineno(CLUREF e, CLUREF num)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(137);

  LINE(138);
    {
    g_envOPlineno.num = num.num;
    }

  LINE(139);
    {
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE set_lineno ****/


/**** BEGIN PROCEDURE get_lineno ****/


errcode
g_envOPget_lineno(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(142);

  LINE(143);
    {
    {
    ret_1->num = g_envOPlineno.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_lineno ****/


/**** BEGIN PROCEDURE get_errst ****/


errcode
g_envOPget_errst(CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(146);

  LINE(147);
    {
    {
    CLUREF T_1_1;
    T_1_1.num = g_envOPcurrent_env.vec->data[6];
    ret_1->num = T_1_1.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_errst ****/


/**** BEGIN PROCEDURE min_inline ****/


errcode
g_envOPmin_inline(CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(150);

  LINE(151);
    {
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = g_envOPcurrent_env.vec->data[12];
    T_1_2.num = T_1_1.num ^ 1;
    ret_1->num = T_1_2.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE min_inline ****/


/**** BEGIN PROCEDURE putl ****/

static int g_envOPputl_own_init = 0;

errcode
g_envOPputl(CLUREF e, CLUREF s)
{
    errcode err;
    if (g_envOPputl_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPputl_own_init = 1;
    }
    enter_proc(154);

  LINE(155);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[5];
    if (T_1_1.num == true) {

  LINE(156);
        {
        err = g_envOPnewline(e);
        if (err != ERR_ok) goto ex_0;
        }
        }
        }/* end if */

  LINE(158);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[3];
    {
    if ((T_1_1.array->int_low + T_1_1.array->ext_size + 1) < T_1_1.array->int_size) {
        T_1_1.array->store->data[T_1_1.array->int_low + T_1_1.array->ext_size] = s.num;
        T_1_1.array->ext_size++; T_1_1.array->ext_high++;}
    else {
        err = arrayOPaddh(T_1_1, s);
        if (err != ERR_ok) goto ex_0;}
    }
    }

  LINE(162);
    {
    err = g_envOPoutput_line(e);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(163);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
    CLUREF T_1_4;
    CLUREF T_1_5;
    T_1_1.num = e.vec->data[4];
    err = stringOPsize(T_1_1, &T_1_2);
    if (err != ERR_ok) goto ex_0;
    T_1_3.num = e.vec->data[8];
    T_1_4.num = (T_1_2.num == T_1_3.num)? true : false;
    T_1_5.num = T_1_4.num ^ 1;
    if (T_1_5.num == true) {

  LINE(164);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        T_2_1.num = e.vec->data[8];
        err = stringOPsubstr(STR__040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040, CLU_1, T_2_1, &T_2_2);
        if (err != ERR_ok) goto ex_0;
        e.vec->data[4] = T_2_2.num;
        }
        }
        }/* end if */

  LINE(165);
    {
    e.vec->data[5] = false;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE putl ****/


/**** BEGIN PROCEDURE puts ****/


errcode
g_envOPputs(CLUREF e, CLUREF s)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(169);

  LINE(170);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[5];
    if (T_1_1.num == true) {

  LINE(171);
        {
        err = g_envOPnewline(e);
        if (err != ERR_ok) goto ex_0;
        }
        }
        }/* end if */

  LINE(173);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[3];
    {
    if ((T_1_1.array->int_low + T_1_1.array->ext_size + 1) < T_1_1.array->int_size) {
        T_1_1.array->store->data[T_1_1.array->int_low + T_1_1.array->ext_size] = s.num;
        T_1_1.array->ext_size++; T_1_1.array->ext_high++;}
    else {
        err = arrayOPaddh(T_1_1, s);
        if (err != ERR_ok) goto ex_0;}
    }
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE puts ****/


/**** BEGIN PROCEDURE newline ****/

static int g_envOPnewline_own_init = 0;

errcode
g_envOPnewline(CLUREF e)
{
    errcode err;
    if (g_envOPnewline_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPnewline_own_init = 1;
    }
    enter_proc(176);

  LINE(178);
    {
    err = g_envOPoutput_line(e);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(179);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
    CLUREF T_1_4;
    CLUREF T_1_5;
    T_1_1.num = e.vec->data[4];
    err = stringOPsize(T_1_1, &T_1_2);
    if (err != ERR_ok) goto ex_0;
    T_1_3.num = e.vec->data[8];
    T_1_4.num = (T_1_2.num == T_1_3.num)? true : false;
    T_1_5.num = T_1_4.num ^ 1;
    if (T_1_5.num == true) {

  LINE(180);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        T_2_1.num = e.vec->data[8];
        err = stringOPsubstr(STR__040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040, CLU_1, T_2_1, &T_2_2);
        if (err != ERR_ok) goto ex_0;
        e.vec->data[4] = T_2_2.num;
        }
        }
        }/* end if */

  LINE(181);
    {
    e.vec->data[5] = false;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE newline ****/


/**** BEGIN PROCEDURE blankline ****/

static int g_envOPblankline_own_init = 0;

errcode
g_envOPblankline(CLUREF e)
{
    errcode err;
    if (g_envOPblankline_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPblankline_own_init = 1;
    }
    enter_proc(184);

  LINE(185);
    {
    err = g_envOPoutput_line(e);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(186);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[9];
    err = streamOPputl(T_1_1, STR_);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(187);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
    CLUREF T_1_4;
    CLUREF T_1_5;
    T_1_1.num = e.vec->data[4];
    err = stringOPsize(T_1_1, &T_1_2);
    if (err != ERR_ok) goto ex_0;
    T_1_3.num = e.vec->data[8];
    T_1_4.num = (T_1_2.num == T_1_3.num)? true : false;
    T_1_5.num = T_1_4.num ^ 1;
    if (T_1_5.num == true) {

  LINE(188);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        T_2_1.num = e.vec->data[8];
        err = stringOPsubstr(STR__040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040, CLU_1, T_2_1, &T_2_2);
        if (err != ERR_ok) goto ex_0;
        e.vec->data[4] = T_2_2.num;
        }
        }
        }/* end if */

  LINE(189);
    {
    e.vec->data[5] = false;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE blankline ****/


/**** BEGIN PROCEDURE ensure_newline ****/


errcode
g_envOPensure_newline(CLUREF e)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(192);

  LINE(194);
    {
    e.vec->data[5] = true;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE ensure_newline ****/


/**** BEGIN PROCEDURE indent ****/

static int g_envOPindent_own_init = 0;

errcode
g_envOPindent(CLUREF e)
{
    errcode err;
    if (g_envOPindent_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPindent_own_init = 1;
    }
    enter_proc(197);

  LINE(200);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = e.vec->data[8];
    T_1_2.num = T_1_1.num + 4;
     if ((T_1_2.num > 0 && T_1_1.num < 0 && 4 < 0) ||
         (T_1_2.num < 0 && T_1_1.num > 0 && 4 > 0)) {
        err = ERR_overflow;
        goto ex_0;}
    e.vec->data[8] = T_1_2.num;
    }

  LINE(201);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = e.vec->data[3];
    T_1_2.num = (T_1_1.array->ext_size == 0)? true : false;
    if (T_1_2.num == true) {

  LINE(202);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        T_2_1.num = e.vec->data[8];
        err = stringOPsubstr(STR__040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040, CLU_1, T_2_1, &T_2_2);
        if (err != ERR_ok) goto ex_0;
        e.vec->data[4] = T_2_2.num;
        }
        }
        }/* end if */
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE indent ****/


/**** BEGIN PROCEDURE outdent ****/

static int g_envOPoutdent_own_init = 0;

errcode
g_envOPoutdent(CLUREF e)
{
    errcode err;
    if (g_envOPoutdent_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPoutdent_own_init = 1;
    }
    enter_proc(206);

  LINE(209);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = e.vec->data[8];
    T_1_2.num = T_1_1.num - 4;
     if ((T_1_2.num >= 0 && T_1_1.num < 0 && (-4) < 0) ||
         (T_1_2.num <= 0 && T_1_1.num > 0 && (-4) > 0)) {
        err = ERR_overflow;
        goto ex_0;}
    e.vec->data[8] = T_1_2.num;
    }

  LINE(210);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = e.vec->data[3];
    T_1_2.num = (T_1_1.array->ext_size == 0)? true : false;
    if (T_1_2.num == true) {

  LINE(211);
        {
        CLUREF T_2_1;
        CLUREF T_2_2;
        T_2_1.num = e.vec->data[8];
        err = stringOPsubstr(STR__040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040_040, CLU_1, T_2_1, &T_2_2);
        if (err != ERR_ok) goto ex_0;
        e.vec->data[4] = T_2_2.num;
        }
        }
        }/* end if */
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE outdent ****/


/**** BEGIN PROCEDURE newdefn ****/

static int g_envOPnewdefn_own_init = 0;

errcode
g_envOPnewdefn(CLUREF e)
{
    errcode err;
    if (g_envOPnewdefn_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPnewdefn_own_init = 1;
    }
    enter_proc(215);

  LINE(217);
    {
    err = g_envOPoutput_line(e);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(218);
    {
    CLUREF T_2_1;
    T_2_1.num = e.vec->data[9];
    err = streamOPputl(T_2_1, STR_);
    if (err != ERR_ok) goto ex_1;
    }
        goto end_1;
        ex_1:
            if ((err == ERR_not_possible)) {

  LINE(219);
                {
                e.vec->data[7] = 2;
                }
            }
            else {
                goto ex_0;
            }
        end_1:;

  LINE(220);
    {
    e.vec->data[4] = STR_.num;
    }

  LINE(221);
    {
    e.vec->data[8] = 0;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE newdefn ****/


/**** BEGIN PROCEDURE error ****/


errcode
g_envOPerror(CLUREF e, CLUREF s)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(224);

  LINE(225);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = e.vec->data[6];
    err = intOPunparse(g_envOPlineno, &T_1_2);
    if (err != ERR_ok) goto ex_0;
    err = streamOPputs(T_1_1, T_1_2);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(226);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = e.vec->data[6];
    T_1_2.ch = ':';
    err = streamOPputc(T_1_1, T_1_2);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(227);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    T_1_1.num = e.vec->data[6];
    T_1_2.ch = '\t';
    err = streamOPputc(T_1_1, T_1_2);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(228);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[6];
    err = streamOPputl(T_1_1, s);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(229);
    {
    e.vec->data[7] = 2;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE error ****/


/**** BEGIN PROCEDURE warn ****/


errcode
g_envOPwarn(CLUREF e, CLUREF s)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(232);

  LINE(233);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[6];
    err = streamOPputl(T_1_1, s);
    if (err != ERR_ok) goto ex_0;
    }

  LINE(234);
    {
    e.vec->data[7] = 0;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE warn ****/


/**** BEGIN PROCEDURE set_convert_type ****/


errcode
g_envOPset_convert_type(CLUREF e, CLUREF t)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(238);

  LINE(239);
    {
    CLUREF T_1_1;
    CellAlloc(2, t.num, T_1_1);
    e.vec->data[1] = T_1_1.num;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE set_convert_type ****/


/**** BEGIN PROCEDURE set_convert ****/


errcode
g_envOPset_convert(CLUREF e, CLUREF flag)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(242);

  LINE(243);
    {
    e.vec->data[2] = flag.num;
    }
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE set_convert ****/


/**** BEGIN PROCEDURE get_convert ****/


errcode
g_envOPget_convert(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    CLUREF t;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(246);

  LINE(247);
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[2];
    if (T_1_1.num == true) {

  LINE(248);
        {
        CLUREF T_2_1;
        T_2_1.num = e.vec->data[1];
        switch (T_2_1.cell->tag) {
        case 1:
             {

  LINE(249);
                {
                {signal (ERR_none);}}
                break;
                }
        case 2:
             {CLUREF T_2_2;
            T_2_2.num = T_2_1.cell->value;
            t.num = T_2_2.num;

  LINE(250);
                {
                {
                ret_1->num = t.num;
                }
                {signal (ERR_ok);}}
                break;
                }
        }
        }
        }
    else {

  LINE(252);
        {
        {signal (ERR_none);}}
        }}/* end if */
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_convert ****/


/**** BEGIN PROCEDURE output_line ****/

static int g_envOPoutput_line_own_init = 0;

errcode
g_envOPoutput_line(CLUREF e)
{
    errcode err;
    CLUREF each_s;
    if (g_envOPoutput_line_own_init == 0) {
        if (g_env_own_init == 0) {
            err = g_env_own_init_proc();
            if (err != ERR_ok) goto ex_0;
        }
        g_envOPoutput_line_own_init = 1;
    }
    enter_proc(261);

  LINE(263);
    {
    CLUREF T_2_1;
    CLUREF T_2_2;
    CLUREF T_2_3;
    T_2_1.num = e.vec->data[3];
    T_2_2.num = (T_2_1.array->ext_size == 0)? true : false;
    T_2_3.num = T_2_2.num ^ 1;
    if (T_2_3.num == true) {

  LINE(265);
        {
        CLUREF T_3_1;
        CLUREF T_3_2;
        T_3_1.num = (g_envOPlineno.num == g_envOPlast_lineno.num)? true : false;
        T_3_2.num = T_3_1.num ^ 1;
        if (T_3_2.num == true) {

  LINE(266);
            {
            CLUREF T_4_1;
            T_4_1.num = e.vec->data[9];
            err = streamOPputl(T_4_1, STR_);
            if (err != ERR_ok) goto ex_1;
            }

  LINE(267);
            {
            CLUREF T_4_1;
            CLUREF T_4_2;
            T_4_1.num = (g_envOPlineno.num == 0)? true : false;
            T_4_2.num = T_4_1.num ^ 1;
            if (T_4_2.num == true) {

  LINE(269);
                {
                CLUREF T_5_1;
                err = ibOPbodies(&T_5_1);
                if (err != ERR_ok) goto ex_1;
                if (T_5_1.num == true) {

  LINE(271);
                    {
                    CLUREF T_6_1;
                    CLUREF T_6_2;
                    CLUREF T_6_3;
                    CLUREF T_6_4;
                    T_6_1.num = e.vec->data[9];
                    err = intOPunparse(g_envOPlineno, &T_6_2);
                    if (err != ERR_ok) goto ex_1;
                    err = stringOPconcat(STR__040_040FB_137LINE_050, T_6_2, &T_6_3);
                    if (err != ERR_ok) goto ex_1;
                    err = stringOPconcat(T_6_3, STR__051_073, &T_6_4);
                    if (err != ERR_ok) goto ex_1;
                    err = streamOPputl(T_6_1, T_6_4);
                    if (err != ERR_ok) goto ex_1;
                    }
                    }
                else {

  LINE(275);
                    {
                    CLUREF T_6_1;
                    CLUREF T_6_2;
                    CLUREF T_6_3;
                    CLUREF T_6_4;
                    T_6_1.num = e.vec->data[9];
                    err = intOPunparse(g_envOPlineno, &T_6_2);
                    if (err != ERR_ok) goto ex_1;
                    err = stringOPconcat(STR__040_040LINE_050, T_6_2, &T_6_3);
                    if (err != ERR_ok) goto ex_1;
                    err = stringOPconcat(T_6_3, STR__051_073, &T_6_4);
                    if (err != ERR_ok) goto ex_1;
                    err = streamOPputl(T_6_1, T_6_4);
                    if (err != ERR_ok) goto ex_1;
                    }
                    }}/* end if */
                }
                }/* end if */

  LINE(282);
            {
            g_envOPlast_lineno.num = g_envOPlineno.num;
            }
            }
            }/* end if */

  LINE(284);
        {
        CLUREF T_3_1;
        CLUREF T_3_2;
        T_3_1.num = e.vec->data[9];
        T_3_2.num = e.vec->data[4];
        err = streamOPputs(T_3_1, T_3_2);
        if (err != ERR_ok) goto ex_1;
        }

  LINE(285);
        {
        CLUREF T_3_1;
        CLUREF T_3_2;
        CLUREF T_3_3;
        CLUREF T_3_4;
            T_3_1.num = e.vec->data[3];
            T_3_3.num = T_3_1.array->ext_high;
            T_3_4 = T_3_1;
            for (T_3_2.num = T_3_1.array->ext_low; T_3_2.num <= T_3_3.num; T_3_2.num++) {
                if (T_3_2.num > T_3_4.array->ext_high || T_3_2.num < T_3_4.array->ext_low) {
                    err = ERR_failure;
                    elist[0] = array_bounds_exception_STRING;
                    goto ex_1;}
                each_s.num = T_3_4.array->store->data[T_3_2.num - T_3_4.array->ext_low + T_3_4.array->int_low];

  LINE(286);
                {
                CLUREF T_4_1;
                T_4_1.num = e.vec->data[9];
                err = streamOPputs(T_4_1, each_s);
                if (err != ERR_ok) goto ex_1;
                }
            }
        }
        end_inline_for_1:;

  LINE(288);
        {
        CLUREF T_3_1;
        T_3_1.num = e.vec->data[9];
        err = streamOPputl(T_3_1, STR_);
        if (err != ERR_ok) goto ex_1;
        }

  LINE(289);
        {
        CLUREF T_3_1;
        T_3_1.num = e.vec->data[3];
        err = arrayOPtrim(T_3_1, CLU_1, CLU_0);
        if (err != ERR_ok) goto ex_1;
        }
        }
        }/* end if */
        goto end_1;
        ex_1:
            if ((err == ERR_not_possible)) {

  LINE(291);
                {
                e.vec->data[7] = 2;
                }
            }
            else {
                goto ex_0;
            }
        end_1:;
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: {signal(ERR_ok);}
}

/**** END PROCEDURE output_line ****/


/**** BEGIN PROCEDURE get_comp ****/


errcode
g_envOPget_comp(CLUREF e, CLUREF *ret_1)
{
    errcode err;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(294);

  LINE(295);
    {
    {
    CLUREF T_1_1;
    T_1_1.num = e.vec->data[0];
    ret_1->num = T_1_1.num;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE get_comp ****/


/**** BEGIN PROCEDURE add_typedef ****/


errcode
g_envOPadd_typedef(CLUREF id, CLUREF *ret_1)
{
    errcode err;
    CLUREF each_id;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(298);

  LINE(300);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
        T_1_2.num = g_envOPtypedef_list.vec->size;
        T_1_3 = g_envOPtypedef_list;
        for (T_1_1.num = 1; T_1_1.num <= T_1_2.num; T_1_1.num++) {
            each_id.num = T_1_3.vec->data[T_1_1.num - 1];

  LINE(301);
            {
            CLUREF T_2_1;
            CLUREF T_2_2;
            CLUREF T_2_3;
            err = idnOPget_str(id, &T_2_1);
            if (err != ERR_ok) goto ex_0;
            err = idnOPget_str(each_id, &T_2_2);
            if (err != ERR_ok) goto ex_0;
            T_2_3.num = ((T_2_1.str->size != T_2_2.str->size)? false :
                !(memcmp(T_2_1.str->data, T_2_2.str->data, T_2_1.str->size)));
            if (T_2_3.num == true) {
                {
                {
                ret_1->tf = false;
                }
                {signal (ERR_ok);}}
                }
                }/* end if */
        }
    }
    end_inline_for_1:;

  LINE(303);
    {
    CLUREF T_1_1;
    err = sequenceOPaddh(g_envOPtypedef_list, id, &T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPtypedef_list.num = T_1_1.num;
    }

  LINE(304);
    {
    {
    ret_1->tf = true;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE add_typedef ****/


/**** BEGIN PROCEDURE add_global ****/


errcode
g_envOPadd_global(CLUREF nm, CLUREF *ret_1)
{
    errcode err;
    CLUREF each_nm;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(307);

  LINE(309);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
        T_1_2.num = g_envOPglobal_list.vec->size;
        T_1_3 = g_envOPglobal_list;
        for (T_1_1.num = 1; T_1_1.num <= T_1_2.num; T_1_1.num++) {
            each_nm.num = T_1_3.vec->data[T_1_1.num - 1];

  LINE(310);
            {
            CLUREF T_2_1;
            T_2_1.num = ((nm.str->size != each_nm.str->size)? false :
                !(memcmp(nm.str->data, each_nm.str->data, nm.str->size)));
            if (T_2_1.num == true) {
                {
                {
                ret_1->tf = false;
                }
                {signal (ERR_ok);}}
                }
                }/* end if */
        }
    }
    end_inline_for_1:;

  LINE(312);
    {
    CLUREF T_1_1;
    err = sequenceOPaddh(g_envOPglobal_list, nm, &T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPglobal_list.num = T_1_1.num;
    }

  LINE(313);
    {
    {
    ret_1->tf = true;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE add_global ****/


/**** BEGIN PROCEDURE add_str ****/


errcode
g_envOPadd_str(CLUREF nm, CLUREF *ret_1)
{
    errcode err;
    CLUREF each_nm;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(316);

  LINE(318);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
        T_1_2.num = g_envOPstr_list.vec->size;
        T_1_3 = g_envOPstr_list;
        for (T_1_1.num = 1; T_1_1.num <= T_1_2.num; T_1_1.num++) {
            each_nm.num = T_1_3.vec->data[T_1_1.num - 1];

  LINE(319);
            {
            CLUREF T_2_1;
            T_2_1.num = ((nm.str->size != each_nm.str->size)? false :
                !(memcmp(nm.str->data, each_nm.str->data, nm.str->size)));
            if (T_2_1.num == true) {
                {
                {
                ret_1->tf = false;
                }
                {signal (ERR_ok);}}
                }
                }/* end if */
        }
    }
    end_inline_for_1:;

  LINE(321);
    {
    CLUREF T_1_1;
    err = sequenceOPaddh(g_envOPstr_list, nm, &T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPstr_list.num = T_1_1.num;
    }

  LINE(322);
    {
    {
    ret_1->tf = true;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE add_str ****/


/**** BEGIN PROCEDURE add_extern ****/


errcode
g_envOPadd_extern(CLUREF nm, CLUREF *ret_1)
{
    errcode err;
    CLUREF each_nm;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(325);

  LINE(327);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
        T_1_2.num = g_envOPextern_list.vec->size;
        T_1_3 = g_envOPextern_list;
        for (T_1_1.num = 1; T_1_1.num <= T_1_2.num; T_1_1.num++) {
            each_nm.num = T_1_3.vec->data[T_1_1.num - 1];

  LINE(328);
            {
            CLUREF T_2_1;
            T_2_1.num = ((each_nm.str->size != nm.str->size)? false :
                !(memcmp(each_nm.str->data, nm.str->data, each_nm.str->size)));
            if (T_2_1.num == true) {
                {
                {
                ret_1->tf = false;
                }
                {signal (ERR_ok);}}
                }
                }/* end if */
        }
    }
    end_inline_for_1:;

  LINE(330);
    {
    CLUREF T_1_1;
    err = sequenceOPaddh(g_envOPextern_list, nm, &T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPextern_list.num = T_1_1.num;
    }

  LINE(331);
    {
    {
    ret_1->tf = true;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE add_extern ****/


/**** BEGIN PROCEDURE add_vlist ****/


errcode
g_envOPadd_vlist(CLUREF n, CLUREF *ret_1)
{
    errcode err;
    CLUREF each_n;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(334);

  LINE(335);
    {
    CLUREF T_1_1;
    T_1_1.num = (n.num < 50)? true : false;
    if (T_1_1.num == true) {
        {
        {
        ret_1->tf = false;
        }
        {signal (ERR_ok);}}
        }
        }/* end if */

  LINE(336);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
        T_1_2.num = g_envOPvlist.vec->size;
        T_1_3 = g_envOPvlist;
        for (T_1_1.num = 1; T_1_1.num <= T_1_2.num; T_1_1.num++) {
            each_n.num = T_1_3.vec->data[T_1_1.num - 1];

  LINE(337);
            {
            CLUREF T_2_1;
            T_2_1.num = (each_n.num == n.num)? true : false;
            if (T_2_1.num == true) {
                {
                {
                ret_1->tf = false;
                }
                {signal (ERR_ok);}}
                }
                }/* end if */
        }
    }
    end_inline_for_1:;

  LINE(339);
    {
    CLUREF T_1_1;
    err = sequenceOPaddh(g_envOPvlist, n, &T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPvlist.num = T_1_1.num;
    }

  LINE(340);
    {
    {
    ret_1->tf = true;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE add_vlist ****/


/**** BEGIN PROCEDURE add_siglist ****/


errcode
g_envOPadd_siglist(CLUREF n, CLUREF *ret_1)
{
    errcode err;
    CLUREF each_n;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(343);

  LINE(344);
    {
    CLUREF T_1_1;
    T_1_1.num = (n.num < 11)? true : false;
    if (T_1_1.num == true) {
        {
        {
        ret_1->tf = false;
        }
        {signal (ERR_ok);}}
        }
        }/* end if */

  LINE(345);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
        T_1_2.num = g_envOPvlist.vec->size;
        T_1_3 = g_envOPvlist;
        for (T_1_1.num = 1; T_1_1.num <= T_1_2.num; T_1_1.num++) {
            each_n.num = T_1_3.vec->data[T_1_1.num - 1];

  LINE(346);
            {
            CLUREF T_2_1;
            T_2_1.num = (each_n.num == n.num)? true : false;
            if (T_2_1.num == true) {
                {
                {
                ret_1->tf = false;
                }
                {signal (ERR_ok);}}
                }
                }/* end if */
        }
    }
    end_inline_for_1:;

  LINE(348);
    {
    CLUREF T_1_1;
    err = sequenceOPaddh(g_envOPsiglist, n, &T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPsiglist.num = T_1_1.num;
    }

  LINE(349);
    {
    {
    ret_1->tf = true;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE add_siglist ****/


/**** BEGIN PROCEDURE add_find_owns ****/


errcode
g_envOPadd_find_owns(CLUREF nm, CLUREF *ret_1)
{
    errcode err;
    CLUREF each_nm;
    if (g_env_own_init == 0) {
        err = g_env_own_init_proc();
        if (err != ERR_ok) goto ex_0;
    }
    enter_proc(352);

  LINE(354);
    {
    CLUREF T_1_1;
    CLUREF T_1_2;
    CLUREF T_1_3;
        T_1_2.num = g_envOPfind_owns_list.vec->size;
        T_1_3 = g_envOPfind_owns_list;
        for (T_1_1.num = 1; T_1_1.num <= T_1_2.num; T_1_1.num++) {
            each_nm.num = T_1_3.vec->data[T_1_1.num - 1];

  LINE(355);
            {
            CLUREF T_2_1;
            T_2_1.num = ((each_nm.str->size != nm.str->size)? false :
                !(memcmp(each_nm.str->data, nm.str->data, each_nm.str->size)));
            if (T_2_1.num == true) {
                {
                {
                ret_1->tf = false;
                }
                {signal (ERR_ok);}}
                }
                }/* end if */
        }
    }
    end_inline_for_1:;

  LINE(357);
    {
    CLUREF T_1_1;
    err = sequenceOPaddh(g_envOPfind_owns_list, nm, &T_1_1);
    if (err != ERR_ok) goto ex_0;
    g_envOPfind_owns_list.num = T_1_1.num;
    }

  LINE(358);
    {
    {
    ret_1->tf = true;
    }
    {signal (ERR_ok);}}
    goto end_0;
    ex_0:
        {
            if (err == ERR_failure) {signal(ERR_failure);}
            elist[0] = _pclu_erstr(err);
            {signal(ERR_failure);}
        }
    end_0: elist[0] = no_return_values_STRING;
        {signal(ERR_failure);}
}

/**** END PROCEDURE add_find_owns ****/

typedef struct{
    long count;
    OWNPTR type_owns;
    OWNPTR op_owns;
    struct OP_ENTRY entry[37];
} g_env_OPS;

CLU_proc g_env_oe_add_extern = { .proc = g_envOPadd_extern };
CLU_proc g_env_oe_add_find_owns = { .proc = g_envOPadd_find_owns };
CLU_proc g_env_oe_add_global = { .proc = g_envOPadd_global };
CLU_proc g_env_oe_add_siglist = { .proc = g_envOPadd_siglist };
CLU_proc g_env_oe_add_str = { .proc = g_envOPadd_str };
CLU_proc g_env_oe_add_typedef = { .proc = g_envOPadd_typedef };
CLU_proc g_env_oe_add_vlist = { .proc = g_envOPadd_vlist };
CLU_proc g_env_oe_blankline = { .proc = g_envOPblankline };
CLU_proc g_env_oe_create = { .proc = g_envOPcreate };
CLU_proc g_env_oe_end_all = { .proc = g_envOPend_all };
CLU_proc g_env_oe_ensure_newline = { .proc = g_envOPensure_newline };
CLU_proc g_env_oe_error = { .proc = g_envOPerror };
CLU_proc g_env_oe_force_signal = { .proc = g_envOPforce_signal };
CLU_proc g_env_oe_get_comp = { .proc = g_envOPget_comp };
CLU_proc g_env_oe_get_convert = { .proc = g_envOPget_convert };
CLU_proc g_env_oe_get_err = { .proc = g_envOPget_err };
CLU_proc g_env_oe_get_errst = { .proc = g_envOPget_errst };
CLU_proc g_env_oe_get_file_name = { .proc = g_envOPget_file_name };
CLU_proc g_env_oe_get_lineno = { .proc = g_envOPget_lineno };
CLU_proc g_env_oe_get_space = { .proc = g_envOPget_space };
CLU_proc g_env_oe_get_time = { .proc = g_envOPget_time };
CLU_proc g_env_oe_indent = { .proc = g_envOPindent };
CLU_proc g_env_oe_min_inline = { .proc = g_envOPmin_inline };
CLU_proc g_env_oe_newdefn = { .proc = g_envOPnewdefn };
CLU_proc g_env_oe_newline = { .proc = g_envOPnewline };
CLU_proc g_env_oe_outdent = { .proc = g_envOPoutdent };
CLU_proc g_env_oe_putl = { .proc = g_envOPputl };
CLU_proc g_env_oe_puts = { .proc = g_envOPputs };
CLU_proc g_env_oe_reset = { .proc = g_envOPreset };
CLU_proc g_env_oe_reset_err = { .proc = g_envOPreset_err };
CLU_proc g_env_oe_set_convert = { .proc = g_envOPset_convert };
CLU_proc g_env_oe_set_convert_type = { .proc = g_envOPset_convert_type };
CLU_proc g_env_oe_set_file_name = { .proc = g_envOPset_file_name };
CLU_proc g_env_oe_set_lineno = { .proc = g_envOPset_lineno };
CLU_proc g_env_oe_set_space = { .proc = g_envOPset_space };
CLU_proc g_env_oe_set_time = { .proc = g_envOPset_time };
CLU_proc g_env_oe_warn = { .proc = g_envOPwarn };

g_env_OPS g_env_ops_actual = {37, (OWNPTR)&g_env_own_init, (OWNPTR)&g_env_own_init, {
    {&g_env_oe_add_extern, "add_extern"},
    {&g_env_oe_add_find_owns, "add_find_owns"},
    {&g_env_oe_add_global, "add_global"},
    {&g_env_oe_add_siglist, "add_siglist"},
    {&g_env_oe_add_str, "add_str"},
    {&g_env_oe_add_typedef, "add_typedef"},
    {&g_env_oe_add_vlist, "add_vlist"},
    {&g_env_oe_blankline, "blankline"},
    {&g_env_oe_create, "create"},
    {&g_env_oe_end_all, "end_all"},
    {&g_env_oe_ensure_newline, "ensure_newline"},
    {&g_env_oe_error, "error"},
    {&g_env_oe_force_signal, "force_signal"},
    {&g_env_oe_get_comp, "get_comp"},
    {&g_env_oe_get_convert, "get_convert"},
    {&g_env_oe_get_err, "get_err"},
    {&g_env_oe_get_errst, "get_errst"},
    {&g_env_oe_get_file_name, "get_file_name"},
    {&g_env_oe_get_lineno, "get_lineno"},
    {&g_env_oe_get_space, "get_space"},
    {&g_env_oe_get_time, "get_time"},
    {&g_env_oe_indent, "indent"},
    {&g_env_oe_min_inline, "min_inline"},
    {&g_env_oe_newdefn, "newdefn"},
    {&g_env_oe_newline, "newline"},
    {&g_env_oe_outdent, "outdent"},
    {&g_env_oe_putl, "putl"},
    {&g_env_oe_puts, "puts"},
    {&g_env_oe_reset, "reset"},
    {&g_env_oe_reset_err, "reset_err"},
    {&g_env_oe_set_convert, "set_convert"},
    {&g_env_oe_set_convert_type, "set_convert_type"},
    {&g_env_oe_set_file_name, "set_file_name"},
    {&g_env_oe_set_lineno, "set_lineno"},
    {&g_env_oe_set_space, "set_space"},
    {&g_env_oe_set_time, "set_time"},
    {&g_env_oe_warn, "warn"}}};

struct OPS *g_env_ops = (struct OPS *)&g_env_ops_actual;

/**** END CLUSTER g_env ****/
