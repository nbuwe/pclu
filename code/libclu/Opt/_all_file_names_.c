...
_all_file_names_(CLUREF fs, errcode (*proc)(), void *user_locals, errcode *iecode)
{
  ...
  ex_3: /* resignal */
    ...
    if (locals.err == ERR_not_possible)
        signal(locals.err);
    else if (locals.err == ERR_bad_format)
        signal(locals.err);
    else
        goto ex_0;
  end_3:;
...
_all_file_names__IB_1(CLUREF iv_1, _all_file_names__LOCALS_t *locals, errcode *iecode)
{
  ...
  FB_LINE(58);
    { /* for */
        ...
        if (locals->err == ERR_iteriterbodysignal)
            signal(ERR_iteriterbodysignal);
        if (locals->err == ERR_break && !locals->body_ctrl_req)
            signal(ERR_break);
        if (locals->err == ERR_iteriterbodyexit && !locals->body_ctrl_req)
            signal(ERR_iteriterbodyexit);
        if (locals->err == ERR_break)
            locals->err = ERR_ok;
        if (locals->err == ERR_iteriterbodyexit)
  ...
  FB_LINE(80);
    { /* if */
    CLUREF T_1_1;
    T_1_1.num = !_all_file_names_OPinuse.num;
...
_match_name_spec(CLUREF off, CLUREF buf, CLUREF nm, CLUREF *ret_1)
{
  ...
  LINE(123);
                ...
                CLUREF T_6_4;
                T_6_3.ch = '\000';
                T_6_4.num = (c2.ch == T_6_3.ch);
                if (T_6_4.num == true) { /* elseif */
